...
### 7.3 Row Focus, Row Selection & Detail Panel Sync
...
- **Focused record** — The single record the user is currently browsing. There is exactly zero or one focused record at any time. Focus is indicated by a subtle visual treatment (e.g., light background tint or left-edge accent bar) distinct from the selection highlight.
- **Selected records** — Zero or more records marked for bulk operations. Selected records have a distinct selection highlight and their selection indicators (e.g., checkboxes in grid views) are filled. The Content Tool Bar reflects the selection count using a graduated transition (Section 11.4): selection feedback appears at 1+ selected records, and Bulk Action Mode activates at 2+ selected records.
- **Anchor record** — The focused record serves as the anchor point for all range-selection operations within a view type. Moving focus always moves the anchor.

#### Shell-Level Contracts

**Detail Panel follows focus:**

| Event             | Shell Behavior                                                                                                                                        |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Focus changes** | The Detail Panel (Docked Window) instantly updates to display the newly focused record in Preview Mode.                                               |
| **Focus cleared** | The Docked Window shows an empty state prompt ("Select a record to preview") or closes.                                                               |
| **Prefetching**   | The system prefetches the 2-3 records adjacent to the focused record so that rapid focus navigation triggers instant rendering with no loading delay. |

**Content Tool Bar follows selection:**

| Event                            | Shell Behavior                                                                                                   |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| **Selection count reaches 1**    | Left zone of Content Tool Bar updates to show selection count and management dropdown (Section 11.4, State 2).   |
| **Selection count reaches 2+**   | Content Tool Bar enters Bulk Action Mode — right zone transforms to bulk action buttons (Section 11.4, State 3). |
| **Selection count returns to 0** | Content Tool Bar returns to default state (Section 11.4, State 1).                                               |

**Focus and selection are independent:**

A record can be in any combination of focused and selected states. The visual treatment must clearly distinguish all four states:

| State                            | Visual Treatment                                                                                       |
| -------------------------------- | ------------------------------------------------------------------------------------------------------ |
| **Focused only**                 | Subtle focus indicator (e.g., light background tint or left-edge accent bar). Not selected.            |
| **Selected only**                | Selection highlight (e.g., colored background). Selection indicator filled.                            |
| **Focused and selected**         | Combined treatment — both focus indicator and selection highlight visible. Selection indicator filled. |
| **Neither focused nor selected** | Default appearance. Selection indicator empty.                                                         |

#### Key Principles

These principles apply to all view types that implement focus and selection:

- **Plain interactions never disturb selections.** Browsing through records (changing focus) never modifies the selection set. A user can have records selected for bulk operations and freely browse other records without losing that selection.
- **Focus is the universal anchor.** All range-selection operations anchor from the currently focused record, regardless of the input mechanism. This provides a single, predictable mental model across all view types.
- **Modifier keys bridge the two layers.** View types should provide modifier-key interactions (e.g., Ctrl+Click, Shift+Click) that combine focus movement with selection modification for power users, while also providing dedicated selection controls (e.g., checkboxes) that keep the layers fully separate.
...
The Application Tool Bar is a fixed, full-width bar at the top of the application That resizes with the Content Panelgui-functional-requirements-prd. It provides location context, global search, and access to Help. It spans above all four vertical zones. Settings and Account are accessed from the Entity Bar (Section 5), not from this bar.
...
- There are no icons displayed in any of the action buttons in the content toolbar.
...
The Content Tool Bar uses a **graduated transition** based on how many rows are checkbox-selected (per the Focus vs. Selection model in Section 7.3). The toolbar has three distinct states:

#### State 1 — No Selection (0 checkboxes)

The toolbar displays its default configuration as defined in Section 11.3: master checkbox and column setup on the left, view-scoped quick filter in the center, entity action buttons on the right.

#### State 2 — Single Selection (1 checkbox)

When exactly one row's checkbox is checked, the toolbar provides selection feedback on the left but retains the entity action buttons on the right, since single-record operations are better served by the row context menu (Views & Grid PRD Section 13.4) or the Detail Panel.

**Left — Selection Info:**

- Selection count with dropdown: "1 selected ▾" — dropdown offers Select All, Deselect All, Select All Matching Records (across all pages), Invert Selection
...
- Unchanged — the same entity-specific action buttons from the default state (Section 11.3) remain visible

#### State 3 — Multi-Selection / Bulk Action Mode (2+ checkboxes)

When two or more rows are checkbox-selected, the toolbar fully transitions into **Bulk Action Mode**. The right zone transforms from entity action buttons to bulk action buttons.
...
- **Two primary bulk action buttons** — the most common bulk operations, displayed with record count: `[Bulk Edit (15)]` `[Bulk Tag (15)]`
- **"Other" overflow button** — dropdown menu containing all Bulk Actions including the two Primary actions at the top of the menu separated by a separator bar followed by the remaining bulk actions: Bulk Assign, Bulk Export, Bulk Tag, Bulk Archive, etc. and finally another separator bar and the help function as the last action.

#### Transition Behavior

The transitions between all three states should be smooth — buttons crossfade or transition without jarring layout shifts. The change in button labels and styling (e.g., different color treatment for destructive actions like Bulk Delete) clearly communicates that the toolbar is now operating on the selection. When transitioning from State 3 back to State 2 (e.g., the user unchecks one of two selected rows), the bulk action buttons smoothly revert to the entity action buttons.

**Exiting selection state:** Deselecting all records (via "Deselect All" or unchecking all boxes) returns the toolbar to State 1 (default).

### 11.5 Content Tool Bar Context Menu

Right-clicking or double-clicking anywhere on the Content Tool Bar background displays a context menu providing quick access to view configuration editors. This menu is the same regardless of which zone of the toolbar the user targets — it is not contextual to cursor position.

#### Trigger

- **Right-click** on any area of the Content Tool Bar background (not on a button, checkbox, or search field)
- **Double-click** on any area of the Content Tool Bar background — provides the same menu for users who instinctively double-click rather than right-click

#### Menu Actions

| Action                | Behavior                                                                                                                                                 |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Edit View**         | Opens the full view editor in the Action Panel, showing all view configuration options (columns, filters, sorting, grouping) in one place.               |
| *(separator)*         |                                                                                                                                                          |
| **Edit Columns**      | Opens the column configuration panel directly, allowing the user to add, remove, reorder, and configure columns without navigating the full view editor. |
| **Edit Filters**      | Opens the filter builder directly, showing the current filter tree with the ability to add, modify, or remove filter conditions and groups.              |
| **Edit Sorting**      | Opens the sort configuration panel directly, showing the current sort rules with the ability to add, reorder, or remove sort levels.                     |
| **Edit Grouping**     | Opens the grouping configuration panel directly, showing the current group-by field and sub-group settings.                                              |
| *(separator)*         |                                                                                                                                                          |
| **Edit Grid Display** | Opens the Grid Display Settings panel for the current view, allowing the user to override grid appearance settings (see Section 11.6).                   |

#### Behavior

- Each "Edit" shortcut opens the same panel or section that would be reached through the Action Panel's view configuration, but navigates directly to the relevant section without requiring the user to find it.
- Changes made through these editors follow the same persistence model as changes made through the Action Panel — they are applied as temporary view modifications with a "Modified" indicator, and can be saved or reverted via the Action Panel's view actions.
- The context menu is available in all three Content Tool Bar states (State 1, State 2, State 3). View configuration is always accessible regardless of the current selection state.

### 11.6 Grid Display Settings

Grid Display Settings control the visual appearance of the grid — row density, font size, gridlines, and other rendering options. These settings use a **three-tier cascade**: system defaults are overridden by user preferences, which are overridden by per-view settings.

#### Settings

| Setting                    | Options                                                        | System Default  |
| -------------------------- | -------------------------------------------------------------- | --------------- |
| **Row Density**            | Compact (32-36px) / Standard (40-44px) / Comfortable (48-52px) | Compact         |
| **Font Size**              | Small (12px) / Medium (13px) / Large (14px)                    | Small           |
| **Alternating Row Colors** | On / Off                                                       | On              |
| **Gridlines**              | Both / Horizontal only / Vertical only / None                  | Horizontal only |
| **Row Hover Highlight**    | On / Off                                                       | On              |

#### Three-Tier Cascade

Settings resolve in the following priority order, highest to lowest:

1. **Per-view override** — Stored as part of the view definition. When set, this value is used regardless of the user's preferences or system defaults. Per-view overrides are configured via the Edit Grid Display action (Section 11.5) or within the full view editor.
2. **User preference** — Stored in the user's profile (Settings → Grid Display). Applies to all views that do not have a per-view override. Each user can set their own defaults.
3. **System default** — The values listed in the table above. Used when neither a per-view override nor a user preference is set.

At each tier, settings are independent — a user can override font size without overriding row density, and a view can override alternating row colors without overriding any other setting. Unset values fall through to the next tier.

#### Per-View Override Persistence

Per-view Grid Display overrides follow the same persistence model as other view modifications:

- On a personal view, changes save directly to the view definition.
- On a shared view, changes create a personal override (fork-on-write). Other users see the shared view's settings unchanged.
- When the Edit Grid Display panel is opened, each setting displays its current effective value and indicates which tier it's coming from (e.g., "Compact (view)" vs. "Compact (user default)" vs. "Compact (system default)"). The user can set a per-view override or clear it to fall back to the next tier.

#### Relationship to Section 24.3 (Density Setting)

Section 24.3 defines the density levels and their measurements. This section defines how density and other grid display settings cascade through the three-tier system. The user preference tier in Section 24.3 corresponds to the "User preference" tier here.

### 11.7 Bulk Operation Progress Modal

When a user initiates a bulk action from the Content Tool Bar (Section 11.4), the system presents a **Bulk Operation Progress Modal** — a Floating Modal (Section 14) that manages the full lifecycle of the operation from confirmation through completion. This modal blocks interaction with the underlying workspace until the operation completes or is cancelled.

#### Phase 1 — Confirmation

After the user clicks a bulk action button and configures the operation (e.g., "Set Status = Archived"), a confirmation screen displays:

- **Operation summary:** A clear description of what will happen — e.g., "Set Status to Archived for 15 records"
- **Record count:** Total number of records that will be affected
- **[Confirm]** and **[Cancel]** buttons

Clicking Confirm transitions the modal to Phase 2.

#### Phase 2 — Live Processing

The modal displays a real-time processing view:

- **Progress indicator:** A progress bar or counter at the top — e.g., "Processing 7 of 15..."
- **Record list:** A scrollable list that grows as records are processed. Each row shows:
  - The record's display name (and optionally 1-2 identifying fields for disambiguation)
  - A status indicator: processing spinner → success checkmark (✓) or failure icon (✗)
  - For failed records: the failure reason displayed inline (e.g., "Cannot archive: record has 3 open Tasks linked to it")
- **[Cancel Remaining]** button — Stops processing at the current point. Records already processed (succeeded, skipped, or forced) retain their changes. Unprocessed records are not touched. The modal transitions directly to Phase 4 (Completion Summary) showing which records were processed before cancellation.

Records are processed sequentially so the user can observe progress and intervene if needed.

#### Phase 3 — Failure Disposition

When a record fails, processing **pauses** and the modal presents the failure reason with disposition options:

- **Failure detail:** The record's display name and the specific failure reason
- **Disposition buttons:**
  - **[Skip]** — Leave this record unchanged and continue processing the remaining records
  - **[Force (action)]** — Override the failure and apply the operation anyway (e.g., "Force Archive"). Only available when the failure is a soft constraint (e.g., open linked tasks) rather than a hard constraint (e.g., insufficient permissions). The button label reflects the specific action being forced.
  - **[Cancel Remaining]** — Stop all further processing (same behavior as the Phase 2 cancel)
- **"Apply to all" checkbox:** ☐ "Apply this choice to all [failure reason] failures" — When checked, the selected disposition (Skip or Force) is automatically applied to all subsequent failures that share the same failure reason category. This prevents the user from manually dispositioning dozens of records that all failed for the same reason.

Each failure reason category (e.g., "has open Tasks", "insufficient permissions", "validation error: value out of range") is tracked independently for the "apply to all" mechanism. A permission failure and a validation failure are separate categories — choosing "Skip all" for permission errors does not automatically skip validation errors.

After disposition, processing resumes with the next record.

#### Phase 4 — Completion Summary

After all records have been processed (or after Cancel Remaining), the modal displays a final summary:

- **Outcome counts:** "15 records processed: 12 succeeded, 2 skipped, 1 forced"
- **Failure detail list:** If any records were skipped, a collapsible section lists them with their failure reasons, allowing the user to review what was not changed
- **[Close]** button — Dismisses the modal and returns the user to the grid. The grid refreshes to reflect any changes made during the operation.

#### Modal Behavior Rules

- The modal **cannot be dismissed** during Phases 2 and 3 (processing and disposition). The user must either let the operation complete or use Cancel Remaining.
- The modal **blocks all workspace interaction** while open. The user cannot click grid rows, navigate to other entities, or modify the Content Tool Bar.
- If the user's session disconnects during processing, the server-side operation continues. On reconnection, the modal reappears showing the current state or completed results.
- The grid's data is refreshed when the modal is closed in Phase 4 to reflect all changes.
