...
> 
> **V1.4 (2026-02-22):**
> Restructured Section 13.3 from "Bulk Actions" to "Row Focus, Row Selection & Bulk Actions" — this section is now the authoritative source for grid-specific focus and selection mechanics, implementing the shell-level contracts defined in GUI Functional Requirements PRD V3 Section 7.3. Added Row Focus table (click row body, arrow keys) and expanded Row Selection table to cover checkbox clicks, Ctrl/Cmd+Click, Shift+Click (row body and checkbox), Shift+Arrow, Master checkbox, and Ctrl/Cmd+A. Added Grid-Specific Interaction Principles. Replaced the floating action bar specification with cross-references to the Content Tool Bar's graduated transition (GUI PRD Section 11.4) and Bulk Operation Progress Modal (GUI PRD Section 11.7). Updated Section 13.5 (Keyboard Navigation) to use focus/selection terminology and add Shift+Arrow Up/Down for keyboard-driven selection.
...
| **Row actions**            | See section 13 Grid Interactions below.                             |
...
- **Shift + click column header:** Add as next-level sort (preserves existing sort rules) and displays numbers in each header indicating the order of the sorted columns.
...
| Trigger                         | Action                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Slow double-click cell**      | Activate inline editor for that cell. A slow double-click is two clicks with a deliberate pause between them (above the system double-click speed threshold), distinguished from a fast double-click which opens the record in Modal Full Overlay Window (see Section 13.3). This follows the same pattern as file renaming in desktop operating systems. |
...
When a user needs to see more information than the grid columns show, they can expand a row to display the Detail Panel:

| Property       | Description                                                                                                                                                                                                                                             |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Trigger**    | Click an expand icon on the row, or keyboard shortcut (Space bar with row focused)                                                                                                                                                                      |
| **Rendering**  | A slide-out panel on the right side of the screen (Docked Window per GUI PRD Section 14), or an inline expansion below the row (user preference)                                                                                                        |
| **Content**    | All fields of the entity, including those not in the grid columns. Uses the Card-Based Architecture (GUI PRD Section 15): Identity Card at top, Card Layout Area below with Attribute Cards, Relation Cards, Activity Card, and other configured Cards. |
| **Editing**    | Section-Based Editing within individual Attribute Cards (GUI PRD Section 16). All field types editable.                                                                                                                                                 |
...
The focused row is the single row the user is currently browsing. The Detail Panel instantly displays the focused record. There is exactly zero or one focused row at any time.

| Interaction               | Behavior                                                                                                                                                                                           |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Click row body**        | Sets focus to that row. Detail Panel updates to show that record in Preview Mode. Does NOT affect any checkbox states.                                                                             |
| **Fast double-click row** | Opens the record in a Modal Full Overlay Window (View Mode) for full-screen reading. This is the primary quick-access gesture for viewing a record in detail. Does NOT affect any checkbox states. |
| **Arrow Up / Arrow Down** | Moves focus to the previous/next row. Detail Panel updates instantly. Does NOT affect any checkbox states.                                                                                         |
| **No focus**              | When no row is focused, the Docked Window shows an empty state prompt ("Select a record to preview") or closes.                                                                                    |

#### Row Selection (checkboxes, modifier keys)

Selected rows are zero or more rows whose checkboxes are checked, marking them for bulk operations. The focused row serves as the universal anchor for all Shift-based range operations.

| Interaction                            | Behavior                                                                                                                                                                                                         |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Click checkbox**                     | Toggles that single row's selection state (checked/unchecked). Does NOT move focus. Does NOT update the Detail Panel.                                                                                            |
| **Ctrl/Cmd+Click row body**            | Sets focus to that row (Detail Panel updates) AND toggles that row's checkbox. Power-user shortcut for building a selection without targeting the narrow checkbox.                                               |
| **Shift+Click row body**               | Sets focus to the clicked row (Detail Panel updates) AND selects all rows in the range from the previously focused row to the clicked row (inclusive). Previously checked rows outside the range are unaffected. |
| **Shift+Click checkbox**               | Selects all rows in the range from the currently focused row to the checkbox's row (inclusive). Does NOT move focus. Does NOT update the Detail Panel.                                                           |
| **Shift+Arrow Up/Down**                | Moves focus one row AND toggles the checkbox on each row as focus passes through, enabling keyboard-driven incremental range selection.                                                                          |
| **Master checkbox (Content Tool Bar)** | Selects or deselects all rows on the current page (with option to "Select all N matching records" across all pages). Does NOT move focus.                                                                        |
| **Ctrl/Cmd+A**                         | Selects all rows on the current page. Does NOT move focus.                                                                                                                                                       |

#### Grid-Specific Interaction Principles

- **Plain interactions never disturb selections.** Clicking a row body or using arrow keys to browse never checks or unchecks any checkbox. A user can have 15 rows selected for bulk edit and freely browse other rows without losing that selection.
- **Focus is the universal anchor.** All Shift-based range operations anchor from the currently focused row, regardless of whether the Shift+Click targets the row body or a checkbox.
- **Modifier keys bridge the two layers.** Ctrl+Click and Shift+Click on the row body combine focus movement with selection modification for power users. Checkbox clicks keep the layers fully separate for users who prefer explicit targeting.

#### Available Bulk Actions

When more than one row is selected, the Content Tool Bar enters Bulk Action Mode (GUI PRD Section 11.4, State 3). This mode allows users to perform only the appropriate actions for bulk operations. The available bulk operations are unique to each Entity Type and are defined in each entity's PRD. The general list below should be used if a PRD does not have an Available Actions section.
...
When a bulk action is initiated, the Bulk Operation Progress Modal (GUI PRD Section 11.7) manages the full lifecycle — confirmation, live processing with per-record status, failure disposition with "apply to all" support, and completion summary.

### 13.4 Row Context Menu

Right-clicking a row (or clicking a "..." overflow icon on the row) displays a context menu. The context menu operates in one of two modes depending on whether any checkboxes are selected:

#### Single-Row Mode (0 checkboxes selected)

When no checkboxes are selected, the context menu operates on the right-clicked row:

- **Right-click sets focus** to the right-clicked row. The Detail Panel updates to display that record.
- The menu displays **single-row actions** applicable to that individual record.

**Default single-row actions** (entity PRDs may override with an Available Actions section):

| Action               | Description                                                                                |
| -------------------- | ------------------------------------------------------------------------------------------ |
| Open in Detail Panel | Open the record in the Docked Window (equivalent to clicking the row body)                 |
| Open in Full View    | Open the record in a Modal Full Overlay Window                                             |
| Edit                 | Focus the first editable cell in the row for inline editing                                |
| Duplicate            | Create a copy of the record                                                                |
| Delete               | Delete the record (with confirmation dialog)                                               |
| Copy Link            | Copy a direct URL to this record to the clipboard                                          |
| Assign to...         | Quick-assign to a project, topic, or user via a sub-menu or picker                         |
| Add Tag              | Add a tag to this record via a sub-menu or picker                                          |
| Add Relation         | Create a relation from this record to another entity via a Relation Type and Target picker |

#### Bulk Mode (1+ checkboxes selected)

When any checkboxes are selected, the context menu operates on the **entire selection set** — the rows whose checkboxes are checked:

- **Right-click does NOT move focus.** The Detail Panel remains on whatever record was previously focused.
- The menu displays **bulk actions** applicable to the selection set, regardless of which row the user right-clicked on.
- Selecting a bulk action from the context menu launches the same Bulk Operation Progress Modal (GUI PRD Section 11.7) as the Content Tool Bar's bulk action buttons.

**Default bulk actions** (entity PRDs may override with an Available Bulk Actions section):

| Action          | Description                                                         |
| --------------- | ------------------------------------------------------------------- |
| Bulk Edit Field | Set a field value across all selected records                       |
| Bulk Assign     | Assign selected records to a project, topic, or user                |
| Bulk Tag        | Add/remove tags or select-field values                              |
| Bulk Export     | Export selected records to CSV or other format                      |
| Bulk Delete     | Delete selected records (launches Progress Modal with confirmation) |

#### Entity-Specific Action Overrides

The default action lists above serve as fallbacks. Each entity type's PRD should define its own **Available Actions** (single-row) and **Available Bulk Actions** sections that specify the exact actions, their ordering, and any entity-specific behavior (e.g., "Merge Duplicates" for Contacts, "Bulk Complete" for Tasks). When an entity PRD defines these sections, they replace the defaults above entirely for that entity type.

#### Context Menu and Content Tool Bar Consistency

The bulk actions available in the context menu must always match the bulk actions available in the Content Tool Bar's Bulk Action Mode (GUI PRD Section 11.4, State 3). Both surfaces are alternate paths to the same operations — the context menu is a shortcut, not a separate action set.

### 13.5 Keyboard Navigation

| Key                  | Action                                                                          |
| -------------------- | ------------------------------------------------------------------------------- |
| Arrow keys           | Move focus between cells (Arrow Up/Down move row focus and update Detail Panel) |
...
| Shift+Arrow Up/Down  | Move focus AND toggle checkbox selection on each row as focus passes through    |
| Ctrl/Cmd+A           | Select all rows on the current page (does NOT move focus)                       |
...
### 13.6 Column Header Interactions

#### Double-Click Behavior

Double-clicking a column header provides a fast single-column sort shortcut that clears any existing multi-column sort:

| Interaction                                      | Behavior                                                                               |
| ------------------------------------------------ | -------------------------------------------------------------------------------------- |
| **Double-click column header**                   | Clears all existing sort rules and sets this column as the only sort, ascending (ASC). |
| **Double-click same column header again**        | Flips the sort direction to descending (DESC).                                         |
| **Double-click same column header a third time** | Removes the sort entirely. The view returns to its default sort order.                 |

This provides a fast way to cycle through single-column sorting without opening any menus. The cycle resets if the user double-clicks a different column header (that column becomes the new sole sort, ASC).

Note: Double-clicking the **resize edge** of a column header retains its existing behavior — auto-fit column width to content (Section 8.4).

#### Context Menu

Right-clicking a column header (or clicking a dropdown chevron on the header) displays a context menu for quick, in-context column configuration. This menu provides productivity shortcuts for common column adjustments without requiring the user to open the view editor in the Action Panel.

**Trigger:**

- **Right-click** on any column header
- **Dropdown chevron** — a small arrow icon visible on hover at the right edge of the column header, providing the same menu without requiring a right-click

**Menu Actions:**

| Action                    | Behavior                                                                                                                                                                                            |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Sort Ascending**        | Apply ascending sort on this column. If already sorted ascending, this item appears checked.                                                                                                        |
| **Sort Descending**       | Apply descending sort on this column. If already sorted descending, this item appears checked.                                                                                                      |
| **Remove Sort**           | Remove this column's sort rule. Only visible when this column is actively sorted.                                                                                                                   |
| *(separator)*             |                                                                                                                                                                                                     |
...
| **Formatting**            | Sub-menu with field-type-specific formatting options (see below). Only visible for field types that support formatting.                                                                             |
| *(separator)*             |                                                                                                                                                                                                     |
| **Group by This Column**  | Set this column's field as the group-by field for the current view. If the view is already grouped by this field, the item reads "Remove Grouping."                                                 |
| **Filter by This Column** | Opens the filter builder pre-scoped to this column's field, allowing the user to quickly add a filter condition without selecting the field manually.                                               |

#### Formatting Sub-Menu by Field Type

The Formatting menu item appears only for field types that have configurable display options. For all other field types (Text, Checkbox, Select, Relation, Email, URL, User, Rating), the Formatting item is hidden from the menu.
...
| Negative format     | −100 (minus sign) / (100) (parentheses)    |
| Percentage display  | On (multiplies by 100 and appends %) / Off |
...
| Decimal places      | 0, 1, 2, Auto                                    |
| Thousands separator | On / Off                                         |
| Negative format     | −$100 / ($100)                                   |
| Symbol format       | $1,234 (symbol prefix) / 1,234 USD (code suffix) |

**Date / Datetime fields:**

| Option           | Choices                                                                                                 |
| ---------------- | ------------------------------------------------------------------------------------------------------- |
| Date format      | Feb 22, 2026 / 2/22/26 / 2026-02-22 / 22 Feb 2026                                                       |
| Time component   | Hidden / 12-hour (2:30 PM) / 24-hour (14:30)                                                            |
| Relative display | Absolute (always show date) / Relative ("3 days ago") / Smart (relative within 7 days, absolute beyond) |

**Phone Number fields:**

| Option               | Choices                                         |
| -------------------- | ----------------------------------------------- |
| Format style         | (555) 123-4567 / 555-123-4567 / +1 555 123 4567 |
| International prefix | Show / Hide                                     |
...
| State format | Abbreviated (OH) / Full (Ohio) |
| Country      | Show / Hide                    |

#### Persistence

All changes made through the column header context menu are applied as temporary view modifications (the same fork-on-write model used for filter and sort changes in the Action Panel). A "Modified" indicator appears on the view, and the user can save or revert via the Action Panel's view actions.

#### Relationship to Section 8.4 (Column Operations)

Section 8.4 defines the full set of column operations available in the system. The column header context menu surfaces a subset of those operations as quick shortcuts. Operations not included in the context menu (Add Column, Remove Column, column field mapping) are available through the Columns button in the Content Tool Bar or through the view editor in the Action Panel.
...
| **Row expansion**                             | Opening the Detail Panel for a row to see all fields and rich content via the Card-Based Architecture.                                                                                                                                             |
