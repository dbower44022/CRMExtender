...
> **V1.0 (2026-02-20):**
> This document defines the Adaptive Grid Intelligence system — an intelligent layout engine that automatically optimizes the entire workspace (grid, detail panel, navigation sidebar) based on display characteristics, data content analysis, and user preferences. The system eliminates the need for manual layout configuration by anticipating user needs and adapting the UI to deliver maximum information density on any screen.
>
> This PRD is additive to the [Views & Grid PRD](views-grid-prd_V3.md) and [GUI Functional Requirements PRD](gui-functional-requirements-prd_V1.md). It does not replace any existing specifications — it defines an intelligent optimization layer that operates on top of the existing column system, zone layout, and view persistence model. Where this PRD extends or refines behaviors defined in those PRDs, the changes are called out explicitly in the Cross-PRD Reconciliation section.
>
...
13. [Preview Panel Intelligence](#13-preview-panel-intelligence)
14. [Navigation Sidebar Adaptation](#14-navigation-sidebar-adaptation)
...
**Display-aware workspace allocation** — The layout engine detects the physical display characteristics (resolution, DPI, effective viewport after navigation and panels) and uses this as the foundation for dividing space among the four-zone layout (icon rail, action panel, grid, detail panel). A 4K 27" monitor with 30 columns of data feels completely different from a 1080p laptop showing 5 columns.
...
**Relationship to other PRDs:** This PRD extends the column system defined in the [Views & Grid PRD](views-grid-prd_V3.md), the four-zone layout and responsive breakpoints defined in the [GUI Functional Requirements PRD](gui-functional-requirements-prd_V1.md), and the view persistence model defined in the [Views & Grid PRD, Section 17](views-grid-prd_V3.md#17-view-persistence--sharing).
...
**Preview panels ignore their content.** The detail/preview panel typically receives a fixed percentage of screen width regardless of whether it's displaying a three-line summary or a full conversation timeline. The grid and preview panel don't negotiate — the grid starves while the preview hoards space it isn't using, or vice versa.
...
**G5 — Unified workspace optimization.** The grid, preview panel, and navigation sidebar should function as a single intelligent layout, not independent fixed-width panels.
...
| Preview panel content fit | >80% of preview content visible without scroll on initial load | Content overflow measurement |
...
These personas extend the personas defined in the [Views & Grid PRD, Section 4](views-grid-prd_V3.md#4-user-personas--stories) and the [GUI Functional Requirements PRD, Section 3](gui-functional-requirements-prd_V1.md#3-user-personas--stories).
...
- **US-AGI8:** As a user, I want the preview panel width to adapt based on the complexity of the selected record's content so that the panel uses exactly the space it needs.
- **US-AGI9:** As a user, I want to set a preview panel preference (None, Small, Medium, Large, Huge) in view settings and have the auto-configuration engine respect that preference.
- **US-AGI10:** As a user, I want the grid and preview panel to negotiate space intelligently — if the grid doesn't need much width, the preview should be wider, and vice versa.
...
| Term | Definition |
|---|---|
| **Layout Engine** | The top-level orchestrator that divides the viewport among the four zones (icon rail, action panel, grid, detail panel) based on display analysis, content analysis, and user preferences. Runs on view open and significant resize. |
...
| **Space Budget** | The total available horizontal pixels after subtracting fixed-width elements (icon rail, scrollbar, borders). The space budget is divided among flexible zones (action panel, grid columns, detail panel) by the layout engine. |
...
**The workspace is one budget.** The grid, preview panel, navigation sidebar, and their contents are all participants in a single space allocation algorithm. When one zone is compact, the freed space is redistributed to zones that need it. No zone operates in isolation.
...
Effective Width = Viewport Width
                  - Icon Rail Width (60px, fixed per GUI PRD Section 5)
                  - Vertical Scrollbar (if present, ~15px)
                  - Zone border/divider pixels (~4px total)

Effective Height = Viewport Height
                   - Top Header Bar (48px, fixed per GUI PRD Section 9.4)
                   - Grid Toolbar (40px, fixed per GUI PRD Section 10)
...
|---|---|---|---|
| **Ultra-wide** | ≥2400px | 4K at 1.5x, ultrawide monitors | All zones generous. Preview panel can be wide. Grid can show many columns at comfortable widths. |
| **Spacious** | 1920–2399px | 4K at 2x, 1440p at 1x | Primary target. Full four-zone layout with intelligent allocation. |
...
| **Constrained** | 1024–1439px | Laptop, tablet landscape | Aggressive optimization. Preview panel narrows significantly or becomes overlay. Low-value columns hidden. |
...
The Space Budget Engine is the top-level allocator. It takes the effective viewport dimensions and divides horizontal space among the three flexible zones: action panel, grid, and detail panel. The icon rail is fixed-width and excluded from the budget.
...
**Pass 2 — Within-zone optimization:** The grid zone is subdivided among columns by the Column Width Allocation engine (Section 10). The detail panel zone is optimized by the Preview Panel Intelligence engine (Section 13). The action panel is optimized by the Navigation Sidebar Adaptation engine (Section 14).
...
The user's preview panel preference (Section 18) modifies the detail panel allocation:

| Preview Preference | Detail Panel Adjustment |
|---|---|
...
1. **Fixed zones** — Icon rail (60px), scrollbar, dividers.
...
3. **Preference-driven zones** — The detail panel allocation based on the user's preview preference setting.
...
- A subtle indicator in the grid toolbar shows "3 columns auto-compacted" with a click to expand all.

---

## 13. Preview Panel Intelligence

### 13.1 Overview

The preview panel (Detail Panel, GUI PRD Section 8) participates in the space budget as an intelligent consumer. Instead of maintaining a fixed width, the panel adapts its width and internal density based on display tier, user preference, and the content of the selected record.

### 13.2 Width Determination

The preview panel's width is determined by the space budget engine (Section 8) based on three inputs in priority order:

1. **User preference** — The view-level preview panel setting (None / Small / Medium / Large / Huge) sets the allocation target.
...
The preview panel adapts its internal layout based on the allocated width:

| Allocated Width | Internal Behavior |
|---|---|
| ≥600px | Full three-zone layout (Identity, Context/Notes, Activity Timeline). Activity entries show multi-line content previews. Generous vertical spacing. |
| 480–599px | Standard layout (per current GUI PRD specification). Activity entries show single-line content previews. |
| 360–479px | Compact layout. Zone 2 (Context/Notes) collapses to a summary row. Activity entries show sender + timestamp only, with content on hover. |
| 280–359px | Minimal layout. Identity zone shows name + primary identifier only. Activity is a dense list (sender + time, no content preview). |
| <280px | Panel converts to overlay mode. Click-to-expand from a narrow indicator strip. |

### 13.4 Activity Entry Density

Within the Activity Timeline (Zone 3), the preview panel adapts entry density based on both width and height:
...
- **Content-aware:** Activity entries for Communications include content previews extracted from the communication's `content_clean` field. The preview length adapts to available width: 120+ chars at full width, 60 chars at standard, sender-only at compact.

---

## 14. Navigation Sidebar Adaptation
...
| Standard | Collapsed to overlay by default (per GUI PRD Section 21.3). |
...
The system selects from three density modes (extending the GUI PRD Section 23.3 density settings):
...
If the result set has fewer records than visible rows, the engine does **not** stretch rows to fill the space (this wastes vertical room and looks odd). Instead, the remaining vertical space is available to the preview panel's Activity Timeline, which can expand vertically.
...
9. Run Preview Panel Intelligence (Section 13)
   └── Set internal preview density based on allocated width
...
| Drag splitter (grid/preview) | Splitter position → proportional split (% of flexible zone width) |
...
|---|---|---|---|---|
| **Preview panel size** | Enum | `none`, `small`, `medium`, `large`, `huge` | `medium` | Declares the user's preferred preview panel allocation. The auto-configuration engine uses this as a target, adjustable based on content negotiation. |
...
When a shared view has layout intelligence settings configured by the owner, those settings serve as defaults for all viewers. Each viewer's personal overrides (Section 17) layer on top. A viewer can also override the layout intelligence settings via their personal view override, changing (for example) the preview panel size from the shared view's "large" to their preferred "small."
...
CREATE TABLE user_view_layout_overrides (
    ...
    splitter_pct    NUMERIC,                     -- Grid/preview split as decimal (0.0–1.0)
...
In the user profile settings (GUI PRD Section 18.2), a new "Display Preferences" section:
...
-- Stored as JSONB within the user's preferences:
{
  "display_preferences": {
    ...
    "default_preview_size": "medium"    -- Global default; overridden by per-view settings
  }
}
```
...
| **Preview panel: preference-based with negotiation** | User declares preference (small/medium/large), engine adjusts around it | Combines user intent with intelligent adaptation. The preference is a strong signal, not an absolute constraint. |
...
### 24.1 Views & Grid PRD (views-grid-prd_V3.md)
...
### 24.2 GUI Functional Requirements PRD (gui-functional-requirements-prd_V1.md)

**Section 4.2 — Zone Dimensions:**
The "Default Width" column for each zone is now the initial allocation of the space budget engine at the Spacious display tier. The min/max constraints continue to apply. The Detail Panel's default width (480px) becomes the output of the space budget engine for a "Medium" preview panel preference at the Spacious tier.
...
**Section 21.2 — Breakpoint Definitions:**
...
### 24.3 Custom Objects PRD (custom-objects-prd.md)
...
- Space budget engine with three-zone allocation
...
- View settings extensions (preview panel size, auto-density toggle)
...
- Preview panel internal content adaptation (density modes based on width)
- Navigation sidebar content-aware width
...
|---|---|---|
| **[Views & Grid PRD](views-grid-prd_V3.md)** | Foundation | Column system (Section 8), field type registry (Section 9), view persistence (Section 17), performance targets (Section 19). This PRD extends these foundations. |
| **[GUI Functional Requirements PRD](gui-functional-requirements-prd_V1.md)** | Foundation | Four-zone layout (Section 4), splitter bar (Section 4.3), detail panel (Section 8), responsive breakpoints (Section 21), density settings (Section 23.3). This PRD refines and extends these specifications. |
| **[Custom Objects PRD](custom-objects-prd.md)** | Reference | Field type system (Section 9) informs content analysis heuristics. No modifications to the Custom Objects PRD required. |
...
3. **Shared view layout hints** — When a view owner shares a view, should they be able to attach layout hints (e.g., "this view works best with a large preview panel") that influence the auto-configuration for other users? This would be separate from the layout intelligence settings (which apply to the view itself) — it would be advice to the layout engine about the owner's design intent.
...
| **Space budget** | The total available pixels to be divided among flexible layout zones (action panel, grid, detail panel). |
