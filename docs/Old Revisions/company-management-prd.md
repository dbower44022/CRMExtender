...
> - The `Company Data Model` section in the [Contact Management PRD](contact-management-prd_V3.md) (formerly Section 6) has been replaced with a cross-reference to this document.
...
- **[Views & Grid PRD](views-grid-prd_V3.md)** — Company views, filters, sorts, and inline editing operate on fields defined in the Company field registry. Precomputed scores enable instant sort-by-relationship-strength on company list views.
...
- **Contact identity resolution** — The [Contact Management PRD](contact-management-prd_V3.md) covers probabilistic entity matching for contacts. This PRD focuses on company-level deduplication and intelligence.
...
When a company is resolved from a domain, contacts with email addresses matching that domain are linked via the Contact→Company employment Relation Type (defined in [Contact Management PRD](contact-management-prd_V3.md) Section 5.5):
...
A composite score per company computed from communication patterns. The same scoring model applies to contacts (see [Contact Management PRD](contact-management-prd_V3.md) Section 10.2) with company-specific adjustments for the **breadth** factor (number of distinct contacts at the company in active conversations).
...
Views are stored using the existing views system ([Views & Grid PRD](views-grid-prd_V3.md)). Users can create custom views with their own filter and sort criteria tied to precomputed scores.
...
| **[Contact Management PRD](contact-management-prd_V3.md)** | Contact→Company employment Relation Type is defined in the Contact PRD. Company data model (Contact PRD Section 6) is a cross-reference to this document. | **Bidirectional.** Contact PRD defines employment; this PRD defines the Company entity. |
...
| **[Views & Grid PRD](views-grid-prd_V3.md)** | Company views, filters, sorts use fields from the Company field registry. Precomputed scores enable sort-by-strength. | **Views depend on Company** for field definitions and scores. |
| **[Permissions & Sharing PRD](permissions-sharing-prd_V1.md)** | Company record access, merge permissions, hierarchy management permissions. | **Company depends on Permissions** for access control. |
...
| [Contact Management PRD](contact-management-prd_V3.md) | Contact→Company employment relation; Company section cross-references this document. |
...
| [Views & Grid PRD](views-grid-prd_V3.md) | Company views consume field registry and precomputed scores. |
| [Permissions & Sharing PRD](permissions-sharing-prd_V1.md) | Access control for company records. |
| [Events PRD](events-prd.md) | Companies participate in calendar events. |
| [Email Parsing & Content Extraction](email_stripping.md) | Email signature parsing provides company enrichment data. |
