...
> **V8.0 (2026-02-22):** Added Sortable and Filterable attributes to Core Fields and Computed/Derived Fields tables. Added note on TDD responsibility for documenting technical rationale for non-sortable fields.
...
**Sortable / Filterable columns** declare whether the user can sort and filter by each field in list views and grid context menus. This is a product decision. Fields marked non-sortable are omitted from sort options in column headers and context menus. If a field is marked sortable but is backed by a subquery (noted with †), the Entity TDD must implement a caching or denormalization strategy to ensure acceptable sort performance.

| Field | Description | Required | Sortable | Filterable | Valid Values / Rules |
|---|---|---|---|---|---|
| ID | Unique identifier. Prefixed ULID with `con_` prefix (e.g., `con_01HX7VFBK3...`). Immutable after creation. | Yes | No | Yes | Prefixed ULID |
| First Name | First name or given name. | No | Yes | Yes | Free text |
| Last Name | Last name or family name. | No | Yes | Yes | Free text |
| Display Name | The name shown in all UI contexts. Computed from first + last name unless manually overridden by the user. | Yes | Yes | Yes | Free text, auto-computed |
| Primary Email | The contact's primary email address. Kept in sync with the identifiers model. | No | Yes † | Yes | Valid email format |
| Primary Phone | The contact's primary phone number. Kept in sync with the identifiers model. | No | No | Yes | E.164 format |
| Job Title | Current job title, derived from the most recent current employment record. | No | Yes | Yes | Free text |
| Current Company | Reference to the company the contact currently works for, derived from the most recent current employment record. | No | No | Yes | Reference to Company entity |
| Company Name | The current company's name, maintained for display efficiency. | No | Yes | Yes | Free text, derived from Company |
| Avatar URL | Profile photo. May come from enrichment, social profiles, or manual upload. | No | No | No | Valid URL |
| Lead Source | How this contact first entered the system. | No | Yes | Yes | `email_sync`, `google_contacts`, `csv_import`, `vcard_import`, `linkedin_capture`, `manual`, `enrichment`, `referral` |
| Lead Status | Sales lifecycle stage. | No, defaults to `new` | Yes | Yes | `new`, `contacted`, `qualified`, `nurturing`, `customer`, `lost`, `inactive` |
| Engagement Score | Composite metric reflecting the health and recency of the relationship. Recomputed periodically from behavioral signals. | No, defaults to 0.0 | Yes † | Yes | 0.0–1.0 |
| Intelligence Score | Data completeness metric reflecting how much the system knows about this contact. Recomputed on enrichment, merge, and on a daily schedule. | No, defaults to 0.0 | Yes † | Yes | 0.0–1.0 |
| Source | The first source that created this contact. | No | Yes | Yes | Same values as Lead Source |
| Status | Contact lifecycle status. See Lifecycle section. | Yes, defaults to `active` | Yes | Yes | `active`, `incomplete`, `archived`, `merged` |
| Created By | The user who created the contact, or null for auto-created contacts. | No | No | Yes | Reference to User |
| Created At | When the contact was created. | Yes | Yes | Yes | Timestamp |
| Updated At | When the contact was last modified. | Yes | Yes | Yes | Timestamp |
