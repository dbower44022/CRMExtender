...
## CRMExtender â€” Contact Lifecycle, Identity Resolution & Intelligence Subsystem
...
> **V2.0 Reconciliation Notes (2026-02-17):**
> This version aligns the Contact Management PRD with the [Custom Objects PRD](custom-objects-prd.md), which establishes the Unified Object Model. Key changes:
> - Contact and Company are now **system object types** in the unified framework (`is_system = true`), with core fields protected from deletion and specialized behaviors (identity resolution, enrichment) registered per Custom Objects PRD Section 22.
> - Entity IDs migrated from UUID v4 to **prefixed ULIDs** (`con_` for contacts, `cmp_` for companies) per the platform-wide convention established by the Data Sources PRD and detailed in Custom Objects PRD Section 6.
> - The `custom_fields` JSONB column has been **removed** from both `contacts_current` and `companies_current`. Custom fields on Contacts and Companies are now managed through the **unified field registry** (Custom Objects PRD Section 8), where each custom field becomes a typed column on the entity table via `ALTER TABLE ADD COLUMN`.
> - Employment history is being **reframed as a system Relation Type** (Contact→Company) with temporal metadata, aligning with the first-class Relation Type model (Custom Objects PRD Section 14).
> - The event store is aligned to use **per-entity-type event tables** (`contacts_events`, `companies_events`) rather than a shared event store table, consistent with Custom Objects PRD Section 19.
> - `contacts_current` and `companies_current` are reframed as the Contact and Company object types' **dedicated read model tables**, managed through the object type framework and created by tenant schema provisioning.
...
The Contact Management & Intelligence subsystem is the foundational entity layer of CRMExtender. Every other subsystem â€” conversations, deals, intelligence, relationship graphs â€” ultimately resolves back to contacts and companies. A communication has a sender and recipients. A deal has stakeholders. An intelligence item is about a person or organization. The contact subsystem answers the question: **"Who is this, and what do we know about them?"**

Unlike traditional CRMs that treat contacts as static address book entries, CRMExtender models contacts as **living intelligence objects** that evolve over time. Every field change is an immutable event. Every identifier â€” email, phone, social handle â€” has a lifecycle. Every data point carries source attribution and confidence scoring. The system doesn't just store who someone *is now*; it knows who they *were*, how their profile *changed*, and *why*.

**Core principles:**

- **Event-sourced contacts** â€” All contact and company mutations are stored as immutable events, enabling full audit trails, point-in-time reconstruction, and compliance (GDPR/CCPA data subject access requests).
- **Multi-identifier identity model** â€” A contact is not an email address. A person has multiple emails, phones, social handles, and aliases. The system resolves all of these to a single unified record.
- **Probabilistic entity resolution** â€” When data arrives from multiple sources (email sync, Google Contacts, browser extension, enrichment APIs, manual entry), the system determines whether two records refer to the same real-world person using tiered confidence scoring.
- **Intelligence-first** â€” Contacts are enriched automatically on creation. Behavioral signals, OSINT data, and communication patterns are continuously aggregated. AI generates briefings, suggests tags, and detects anomalies.
- **Temporal awareness** â€” Employment history, contact details, and relationship strengths are tracked over time with valid-from/valid-until bounds. The system knows that Sarah used to work at Acme Corp and now works at Globex, that her old email still resolves, and that her relationship with your team has strengthened over the last quarter.

**Current state:** The PoC implements a minimal contact model â€” a `contacts` table with `email`, `name`, `source`, and `source_id` columns populated from the Google People API. Contacts are used for conversation participant matching and triage filtering. This PRD defines the requirements for the full contact management system.
...
Traditional CRM contact records are snapshots: a name, a company, an email, and a trail of manually logged activities. They degrade silently â€” people change jobs, companies rebrand, phone numbers rotate â€” and the CRM doesn't notice. The record you're looking at might be months or years out of date, and you have no way to know.
...
| **Siloed data sources**       | Contact intelligence exists across Google Contacts, LinkedIn, email signatures, enrichment APIs, and manual notes â€” but no system unifies them with proper attribution and conflict resolution.     |
...
- **Salesforce / HubSpot contacts** â€” Mutable record model. No event history, no point-in-time queries, no built-in enrichment, no graph-based relationships. Duplicates require third-party dedup tools. Custom field hell.
- **LinkedIn** â€” Rich profile data but locked behind their platform. No API access for most users. No integration with personal communication history. No custom intelligence.
- **Enrichment-only tools (Apollo, Clearbit, ZoomInfo)** â€” Provide data snapshots but no lifecycle tracking, no relationship modeling, no communication integration, no AI layer. Must be bolted onto a CRM.
- **Contact management apps (Google Contacts, Apple Contacts)** â€” Address book utilities. No intelligence, no relationships, no business context. Single-identifier model.
- **Graph-based CRMs (Affinity, Dex)** â€” Closer to the target, but limited enrichment, no event sourcing, no temporal history, no multi-channel communication integration.

CRMExtender closes this gap by making contacts **living intelligence objects** â€” event-sourced, multi-identifier, continuously enriched, graph-connected, and AI-analyzed.
...
1. **Unified contact identity** â€” Every person and organization encountered across any channel resolves to a single, canonical contact record via multi-identifier matching.
...
4. **Relationship modeling** â€” Contact-to-contact and contact-to-company relationships are modeled in a graph database with typed, temporal edges and strength scoring.
...
| ID    | Story                                                                                                                                                                       | Acceptance Criteria                                                                                                                               |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| CM-07 | As a user, I want to see a unified contact detail page showing all information about a person â€” profile, communication history, relationships, intelligence â€” in one place. | Contact detail page with sections: Profile, Timeline, Relationships, Intelligence, Employment History, Notes, Deals. All data loads within 200ms. |
...
| CM-09 | As a user, I want to see a contact's full employment history â€” every company, title, and department they've held â€” in chronological order.                                  | Employment history timeline component showing positions with start/end dates, sourced from event store and enrichment.                            |
...
| CM-21 | As a user, I want an AI-generated one-paragraph briefing on any contact that synthesizes all available data â€” profile, communication history, relationship context, recent intelligence. | "Brief me" action on contact detail. Claude generates briefing in < 5 seconds. Briefing cached with TTL. Refresh on demand.          |
...
| CM-23 | As a user, I want to see a visual network graph of a contact's relationships â€” who they know, who they work with, how they connect to my network.       | Interactive graph visualization centered on the selected contact. Zoom, pan, filter by relationship type. Click-through to related contacts. |
...
Contact (event-sourced)
  â”œâ”€â”€ Identity: name, photo, aliases
  â”œâ”€â”€ Classification: tags, categories, lead source, lead status
  â”œâ”€â”€ Custom fields: managed through unified field registry (Custom Objects PRD Section 8)
  â”œâ”€â”€ Computed: engagement_score, intelligence_score
  â”œâ”€â”€ Identifiers (multi-value, lifecycle-tracked)
  â”‚   â”œâ”€â”€ Email addresses (work, personal, old)
  â”‚   â”œâ”€â”€ Phone numbers (mobile, work, home)
  â”‚   â”œâ”€â”€ Social profiles (LinkedIn, Twitter, GitHub)
  â”‚   â””â”€â”€ Any future identifier type
  â”œâ”€â”€ Employment history (temporal)
  â”‚   â””â”€â”€ Company + Title + Department + Start/End
  â”œâ”€â”€ Contact details (multi-value, temporal)
  â”‚   â”œâ”€â”€ Addresses (work, home)
  â”‚   â””â”€â”€ Key dates (birthday, anniversary)
  â”œâ”€â”€ Intelligence items
  â”‚   â””â”€â”€ Enrichment data, OSINT, manual intel
  â”œâ”€â”€ Relationships (graph edges)
  â”‚   â””â”€â”€ Neo4j: KNOWS, REPORTS_TO, INTRODUCED_BY, etc.
  â””â”€â”€ Activity timeline
      â””â”€â”€ Communications, notes, deal events, intel updates
```

```
Company (event-sourced)
  â”œâ”€â”€ Identity: name, domain, logo, industry
  â”œâ”€â”€ Firmographics: size, location, revenue, funding
  â”œâ”€â”€ History: name changes, acquisitions, HQ moves
  â”œâ”€â”€ Employees: contacts with WORKS_AT edges
  â””â”€â”€ Intelligence items
```

```
Group (conventional table)
  â”œâ”€â”€ Name, description, owner
  â””â”€â”€ Members: many-to-many with contacts
```

### 5.2 Contact Record — Read Model Table (System Object Type)

> **V2.0:** The Contact is a **system object type** in the unified framework (`is_system = true`). The `contacts` table (formerly `contacts_current`) is its dedicated read model table, managed through the object type framework within the tenant schema (e.g., `tenant_abc.contacts`). It is created by tenant schema provisioning, not hand-crafted SQL.
...
| `company_id`         | TEXT | FK â†’ `companies_current(id)` | Current company (latest from `employment_history` where `is_current=TRUE`)                                                                 |
...
| `engagement_score`   | REAL | DEFAULT 0.0                  | Composite engagement metric (0.0â€“1.0). Computed from behavioral signals.                                                                   |
| `intelligence_score` | REAL | DEFAULT 0.0                  | Data completeness metric (0.0â€“1.0). How much we know about this contact.                                                                   |
...
| ~~`custom_fields`~~  | —    | **REMOVED in v2.0**          | ~~JSON object of tenant-defined custom field values.~~ **Removed:** Custom fields are now managed through the unified field registry (Custom Objects PRD Section 8). Each custom field is a typed column on the contacts table, added via `ALTER TABLE ADD COLUMN`. See reconciliation notes at top of document.                                                                    |
| `created_by`         | TEXT | FK â†’ `users(id)`             | User who created the contact (NULL for auto-created)                                                                                       |
...
- **Unified Object Model alignment (v2.0):** The `contacts_current` table is the Contact object type's **dedicated read model table**, managed through the object type framework (Custom Objects PRD Section 17). It is created by tenant schema provisioning within the tenant's PostgreSQL schema (e.g., `tenant_abc.contacts`). Core fields listed above are registered as system fields (`is_system = true`) in the field registry and are protected from deletion. Tenants can add custom fields through the unified field registry, which triggers `ALTER TABLE ADD COLUMN` on this table.

### 5.3 Contact Identifiers

Multi-identifier resolution table. Every way a contact can be identified â€” email, phone, social handle, alias â€” is stored here with lifecycle tracking. This is the primary lookup table for resolving incoming communications to contacts.

| Column        | Type    | Constraints                                             | Description                                                                                                                          |
| ------------- | ------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `id`          | TEXT    | **PK**                                                  | UUID v4                                                                                                                              |
| `contact_id`  | TEXT    | NOT NULL, FK â†’ `contacts_current(id)` ON DELETE CASCADE | Which contact this identifier belongs to                                                                                             |
...
| `confidence`  | REAL    | DEFAULT 1.0                                             | Confidence that this identifier belongs to this contact (0.0â€“1.0). Enrichment and auto-detection sources have < 1.0 confidence.      |
...
- `UNIQUE(type, value)` â€” No two contacts can claim the same identifier. If entity resolution discovers a conflict, it triggers a merge workflow.
...
| ------------- | ------- | ------------------------------------------------------- | ----------------------------------------- |
| `id`          | TEXT    | **PK**                                                  | UUID v4                                   |
| `contact_id`  | TEXT    | NOT NULL, FK â†’ `contacts_current(id)` ON DELETE CASCADE |                                           |
...
> **V2.0 Reconciliation Note:** The `employment_history` table is being reframed as a **system Relation Type** (Contact→Company) with `has_metadata = true` and temporal metadata fields (`started_at`, `ended_at`, `title`, `department`, `is_current`, `source`, `confidence`). In the unified model, this relationship is defined as a Relation Type (Custom Objects PRD Section 14) with the Contact object type as source and Company object type as target. The physical storage transitions from this dedicated table to a **relation instance table** managed by the Relation Type framework. The fields and data model below remain accurate for the data captured; the storage mechanism changes. See Custom Objects PRD Section 15 for relation metadata storage patterns.
...
| `id`           | TEXT    | **PK**                                                  | UUID v4                                                                                                   |
| `contact_id`   | TEXT    | NOT NULL, FK â†’ `contacts_current(id)` ON DELETE CASCADE |                                                                                                           |
| `company_id`   | TEXT    | FK â†’ `companies_current(id)` ON DELETE SET NULL         | Linked company record (NULL if company not yet in system)                                                 |
...
### 6.1 Company Record — Read Model Table (System Object Type)

> **V2.0:** Company is a **system object type** (`is_system = true`, prefix `cmp_`). The `companies` table (formerly `companies_current`) is its dedicated read model table within the tenant schema.
...
| ~~`custom_fields`~~ | —    | **REMOVED in v2.0** | ~~JSON object of tenant-defined custom field values.~~ **Removed:** Custom fields managed through unified field registry. See reconciliation notes.                      |
| `created_at`    | TEXT    | NOT NULL    | ISO 8601                                                                                     |
| `updated_at`    | TEXT    | NOT NULL    | ISO 8601                                                                                     |
...
SELECT id FROM companies_current WHERE domain = 'acmecorp.com';

-- 2. If not found, create a minimal company record
INSERT INTO companies_current (id, name, domain, created_at, updated_at)
VALUES (:id, 'acmecorp.com', 'acmecorp.com', :now, :now);
-- Note: name defaults to domain; enrichment will populate the real name.

-- 3. Link contact to company via employment_history
INSERT INTO employment_history (id, contact_id, company_id, company_name, is_current, source)
...
â”œâ”€â”€ email: sarah@acmecorp.com         (active, work, primary, verified, google_contacts)
â”œâ”€â”€ email: sarah.chen@acmecorp.com    (active, work, verified, email_sync)
â”œâ”€â”€ email: sarahc@gmail.com           (active, personal, verified, manual)
â”œâ”€â”€ phone: +15550199                  (active, mobile, primary, enrichment_apollo)
â”œâ”€â”€ linkedin: linkedin.com/in/sarahchen  (active, verified, linkedin_capture)
â””â”€â”€ Employment: VP Engineering @ Acme Corp (current, enrichment_apollo + linkedin_capture)
```
...
| **High**   | Name + Company + Title fuzzy match (>90% similarity) | 0.80â€“0.95          | Auto-merge with review flag |
| **High**   | Name + Email domain match                            | 0.80â€“0.90          | Auto-merge with review flag |
| **Medium** | Name + Company fuzzy match (no title)                | 0.60â€“0.80          | Auto-merge with review flag |
| **Medium** | Name + Location match                                | 0.50â€“0.70          | Queue for human review      |
| **Low**    | Name-only fuzzy match                                | 0.20â€“0.50          | Queue for human review      |
...
â”‚  1. Identifier Extraction        â”‚
â”‚     Extract email, phone, name,  â”‚
â”‚     company, title, social URLs  â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               â”‚
               â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  2. Exact Match Lookup           â”‚
â”‚     Check contact_identifiers    â”‚
â”‚     for email, phone, LinkedIn   â”‚
â”‚     If found â†’ existing contact  â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               â”‚ not found
               â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  3. Fuzzy Match Candidates       â”‚
â”‚     Name + Company similarity    â”‚
â”‚     via Meilisearch or trigram   â”‚
...
â”‚     Score each candidate using   â”‚
â”‚     weighted signal combination  â”‚
â”‚     Apply tenant thresholds      â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               â”‚
  ...
  â”‚ Auto â”‚ â”‚ Auto â”‚ â”‚ Human    â”‚
  â”‚Merge â”‚ â”‚Merge â”‚ â”‚ Review   â”‚
  â”‚      â”‚ â”‚+Flag â”‚ â”‚ Queue    â”‚
  â””â”€â”€â”¬â”€â”€â”€â”˜ â””â”€â”€â”¬â”€â”€â”€â”˜ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
     â”‚         â”‚
     â–¼         â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  5. Merge Execution              â”‚
â”‚     - Pick survivor contact_id   â”‚
â”‚     - Move all identifiers       â”‚
â”‚     - Update participant FKs     â”‚
â”‚     - Emit MergeExecuted event   â”‚
â”‚     - Log in match_candidates    â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```
...
| `confidence_score` | REAL | NOT NULL            | Combined confidence (0.0â€“1.0)                                                                              |
...
| `reviewed_by`      | TEXT | FK â†’ `users(id)`    | User who reviewed (NULL for auto-merged)                                                                   |
| `reviewed_at`      | TEXT |                     | ISO 8601                                                                                                   |
| `created_at`       | TEXT | NOT NULL            | ISO 8601                                                                                                   |

**Constraints:**

- `UNIQUE(entity_type, entity_a_id, entity_b_id)` â€” One candidate record per pair.
...
| `captured_by` | TEXT | FK â†’ `users(id)` | User who captured (NULL for automated sources)                                                          |
...
1. **List selection** â€” Select 2+ contacts via checkboxes on the Contacts list, click "Merge Selected".
2. **Identifier conflict** â€” When adding an email in the Email Addresses section that belongs to another contact, the error message includes a "Merge these contacts?" link. (The generic Identifiers section has been removed from the contact detail page â€” email identifiers are managed via the type-specific Email Addresses section, backed by `contact_identifiers` where `type='email'`.)
...
5. **Post-merge domain resolution:** Resolve email domains from all merged identifiers and auto-create missing company affiliations via `domain_resolver`. This handles the common case where two contacts from different companies are merged â€” both affiliations are preserved.
...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------- |
| `incomplete` | Auto-created from an unknown identifier. Minimal data. Awaiting enrichment.                                                    | â†’ `active` (after enrichment or manual edit) |
| `active`     | Fully identified contact with at least name + one verified identifier.                                                         | â†’ `archived`, â†’ `merged`                     |
| `archived`   | User-archived contact. Excluded from active lists but data preserved. Identifiers still resolve for historical communications. | â†’ `active` (unarchive)                       |
| `merged`     | Duplicate contact that was merged into another record. Soft-deleted.                                                           | â†’ `active` (via split/undo merge)            |
...
â”‚  (Celery task)                   â”‚
â”‚                                  â”‚
â”‚  Selects adapter based on:       â”‚
â”‚  - Available identifiers         â”‚
â”‚  - Adapter priority/cost         â”‚
â”‚  - Rate limits                   â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               â”‚
  ...
  â”‚Apolloâ”‚ â”‚Clear-â”‚ â”‚  People  â”‚ â”‚  Custom   â”‚
  â”‚      â”‚ â”‚ bit  â”‚ â”‚ Data Labsâ”‚ â”‚  Adapter  â”‚
  â””â”€â”€â”¬â”€â”€â”€â”˜ â””â”€â”€â”¬â”€â”€â”€â”˜ â””â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”˜ â””â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”˜
     ...
     â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               ...
               â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  Enrichment Normalizer           â”‚
â”‚  - Maps adapter response to      â”‚
â”‚    common schema                 â”‚
â”‚  - Assigns confidence scores     â”‚
â”‚  - Detects conflicts with        â”‚
â”‚    existing data                 â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               â”‚
               â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  Enrichment Merger               â”‚
â”‚  - Higher-confidence data wins   â”‚
â”‚  - User-entered data always wins â”‚
â”‚  - Source attribution preserved  â”‚
â”‚  - Events emitted for all        â”‚
â”‚    changes                       â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               â”‚
               â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  Intelligence Score Update       â”‚
â”‚  - Recalculate intelligence_scoreâ”‚
â”‚  - Update contacts_current       â”‚
â”‚  - Index in Meilisearch          â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```
...
| ------------- | ---- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id`          | TEXT | **PK**                       | UUID v4                                                                                                                                                  |
| `contact_id`  | TEXT | FK â†’ `contacts_current(id)`  | Linked contact (NULL if company-level intel)                                                                                                             |
| `company_id`  | TEXT | FK â†’ `companies_current(id)` | Linked company (NULL if contact-level intel)                                                                                                             |
...
| `confidence`  | REAL | DEFAULT 1.0                  | Confidence that this intel is accurate (0.0â€“1.0)                                                                                                         |
| `verified_by` | TEXT | FK â†’ `users(id)`             | User who verified (NULL if unverified)                                                                                                                   |
...
|                  | `BOARD_MEMBER_OF`    | `since`, `until`                                     | Board membership (Contactâ†’Company)        |
|                  | `INVESTOR_IN`        | `round`, `amount`                                    | Investment relationship (Contactâ†’Company) |
...
| **Employment**   | `WORKS_AT`           | `role`, `department`, `since`, `until`, `is_current` | Employment (Contactâ†’Company)              |
...
The `KNOWS` relationship includes a `strength` property (0.0â€“1.0) that reflects the depth and recency of the relationship:
...
| ------------------------ | ----------------------------------------------------------------------------- | ----------------------------------------------------- |
| **Engagement score**     | Weighted composite of frequency, recency, and reciprocity across all channels | Overall relationship health (0.0â€“1.0)                 |
...
**Recomputation:** Daily Celery job processes the last 24 hours of communication events and recalculates engagement scores for affected contacts. Scores are persisted on `contacts_current.engagement_score`.
...
| `owner_id`    | TEXT    | FK â†’ `users(id)` | User who created the group                                       |
...
| Column       | Type | Constraints                                             | Description |
| ------------ | ---- | ------------------------------------------------------- | ----------- |
| `group_id`   | TEXT | NOT NULL, FK â†’ `groups(id)` ON DELETE CASCADE           |             |
| `contact_id` | TEXT | NOT NULL, FK â†’ `contacts_current(id)` ON DELETE CASCADE |             |
...
**Smart groups:** When `is_smart=1`, the group's membership is dynamically computed from `query_def` â€” a JSON-serialized filter (same format as saved views). Example:
...
| ------------ | ---- | ------------------------------------------------------- | ------------------------------------------------------ |
| `contact_id` | TEXT | NOT NULL, FK â†’ `contacts_current(id)` ON DELETE CASCADE |                                                        |
| `tag_id`     | TEXT | NOT NULL, FK â†’ `tags(id)` ON DELETE CASCADE             |                                                        |
...
- **Role tags:** Decision Maker, Technical, Executive, Legal, Finance â€” inferred from title and communication patterns.
- **Relationship tags:** VIP, Dormant, New Connection, Frequent Collaborator â€” based on engagement signals.
...
| **vCard 3.0/4.0**       | Apple Contacts, Outlook, Google (export) | Standard contact interchange. Multi-value fields (multiple emails/phones) supported. **Implemented** â€” `poc/vcard_import.py`, web UI at `/contacts/import`, CLI `import-vcards`. Three-tier duplicate detection: email match â†’ normalized phone match â†’ exact name match (customer-scoped). |
...
â”‚     - Parse file format          â”‚
â”‚     - Validate required fields   â”‚
â”‚     - Normalize values           â”‚
â”‚     - Report parsing errors      â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               â”‚
               â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  2. Duplicate Detection          â”‚
â”‚     - Check each record against  â”‚
â”‚       existing contacts via      â”‚
â”‚       entity resolution pipeline â”‚
â”‚     - Classify: new, duplicate,  â”‚
â”‚       update                     â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               â”‚
               â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  3. Preview & User Confirmation  â”‚
â”‚     - Show import preview:       â”‚
â”‚       N new, N updates, N dupes  â”‚
â”‚     - User selects handling for  â”‚
â”‚       each category              â”‚
...
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               ...
               â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  4. Execute Import               â”‚
â”‚     - Create new contacts        â”‚
â”‚     - Merge updates into existingâ”‚
â”‚     - Skip or merge duplicates   â”‚
â”‚     - Emit events for all changesâ”‚
â”‚     - Trigger enrichment for new â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
               â”‚
               â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  5. Import Report                â”‚
â”‚     - Summary: created, updated, â”‚
â”‚       skipped, errored           â”‚
â”‚     - Error details for failed   â”‚
â”‚       records                    â”‚
â”‚     - Import ID for audit trail  â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```
...
You've exchanged 47 emails over the last 6 months â€” your relationship is active and
...
| Warm intro opportunity            | "Sarah knows Marcus Rodriguez at TechVentures â€” they worked together at Globex." |
...
All contact and company mutations are stored as immutable events in an append-only event store. The materialized view tables (`contacts_current`, `companies_current`) are projections derived from these events.

> **V2.0 Alignment:** The Custom Objects PRD (Section 19) establishes **per-entity-type event tables** as the standard pattern. For Contacts, this means a dedicated `contacts_events` table within the tenant schema (e.g., `tenant_abc.contacts_events`). For Companies, `companies_events`. The shared event store table described below is being migrated to this per-entity-type pattern for consistency with the unified model. The event types, payload structure, and snapshot strategy described below remain valid — the change is physical storage location, not event semantics.
...
| `actor_id`        | TEXT    | FK â†’ `users(id)` | User who triggered the event (NULL for system events) |
| `timestamp`       | TEXT    | NOT NULL         | ISO 8601                                              |
| `sequence_number` | INTEGER | NOT NULL         | Monotonically increasing per entity, for ordering     |

**Constraints:**

- `UNIQUE(entity_type, entity_id, sequence_number)` â€” guarantees event ordering per entity.
...
| `IdentifierStatusChanged` | `type`, `value`, `old_status`, `new_status`         | Active â†’ inactive, etc.           |
...
- `contacts_current` â€” Full contact records for the user's accessible contacts.
- `contact_identifiers` â€” For local search and communication participant resolution.
- `employment_history` â€” For contact detail display.
- `companies_current` â€” Company records referenced by contacts.
- `tags` and `contact_tags` â€” For filtering.
- `groups` and `group_members` â€” For group membership display.

**Not synced to client (server-only):**

- Event store â€” Too large, not needed for display.
- Intelligence items â€” Fetched on demand via API.
- Graph relationships â€” Queried live from Neo4j via API.
- Match candidates â€” Admin/review workflow is server-side only.
...
| ---------------------- | --------------------------------------------------------------------- |
| **Right to know**      | Same as GDPR right of access â€” full data export.                      |
| **Right to delete**    | Same as GDPR right to erasure â€” hard delete workflow.                 |
...
### Phase 3 â€” Relationship Intelligence
...
| ~~Custom field builder~~  | ~~P0~~   | ~~Tenant-configurable typed fields~~ **Resolved by Custom Objects PRD:** Custom fields on Contacts (and all entity types) are managed through the unified field registry (Custom Objects PRD Section 8). Each custom field becomes a typed column. No separate "custom field builder" needed — it's the standard field management UI for any object type.                          |
...
| **Apollo**                  | REST API             | Enrichment â†’ contact updates         | Phase 2 |
| **Clearbit**                | REST API             | Enrichment â†’ contact updates         | Phase 2 |
| **People Data Labs**        | REST API             | Enrichment â†’ contact updates         | Phase 2 |
| **LinkedIn (browser ext.)** | Extension â†’ API      | Profile capture â†’ contacts           | Phase 2 |
| **Twitter (browser ext.)**  | Extension â†’ API      | Profile capture â†’ contacts           | Phase 2 |
...
1. **Contact ownership model** â€” Should contacts be owned by individual users, shared across the team, or both (personal vs. shared pools)? Impacts visibility, editing rights, and dedup scope.

2. ~~**Custom field limits**~~ **Resolved by Custom Objects PRD:** 200 user-defined fields per entity type (Custom Objects PRD Section 8.4). Universal fields don't count. Archived fields don't count against the active limit. At 200 columns per table, PostgreSQL performs well for the target record volumes.

3. **Enrichment budget management** â€” Should enrichment API usage have per-tenant monthly budgets? Per-contact caps? How to prioritize enrichment when budget is constrained?

4. **Graph relationship creation** â€” How much relationship creation should be automated (inferred from communication patterns) vs. manual? Automated inference risks false relationships.

5. **Contact photo storage** â€” Store photos in object storage or link to external URLs (Gravatar, LinkedIn)? External URLs break if source changes; local storage costs more.

6. **Multi-tenant contact sharing** â€” For users who belong to multiple tenants, should contacts be shareable across tenants? Impacts data isolation model.

7. **Contact scoring customization** â€” Should tenants be able to customize engagement and intelligence score weights? Adds complexity but increases relevance per use case.

8. **LinkedIn API partnership** â€” LinkedIn's API is restricted. Evaluate Voyager API access, LinkedIn Sales Navigator integration, or browser extension only.

9. **Email signature parsing accuracy** â€” Email signatures contain valuable contact data (name, title, phone, address) but parsing is error-prone. Evaluate ML-based signature parsing vs. regex vs. third-party service.

10. **Historical enrichment** â€” When enrichment data arrives, should it retroactively update intelligence items and scores for historical periods, or only apply going forward?
...
| **Materialized view** (Read model) | A precomputed table derived from event replay, used for fast read access. In the unified model, this is the entity type's dedicated table (e.g., `contacts` table), managed by the object type framework.                                      |
...
| **Intelligence score** | Composite metric (0.0â€“1.0) reflecting how much the system knows about a contact                               |
| **Engagement score**   | Composite metric (0.0â€“1.0) reflecting the health and recency of the relationship                              |
| **Warm intro path**    | A chain of mutual connections between the user and a target contact                                           |
| **OSINT**              | Open-Source Intelligence â€” publicly available data about people and companies                                 |
| **DSAR**               | Data Subject Access Request â€” a GDPR right for individuals to request their data                              |
...
| [Custom Objects PRD](custom-objects-prd.md)                                 | Defines the unified object model, field registry, and relation framework that Contact and Company are system instances of. Governs storage architecture, DDL management, and event sourcing patterns. |
