...
## CRMExtender Ã¢â‚¬â€ Contact Lifecycle, Identity Resolution & Intelligence Subsystem

**Version:** 3.0
**Date:** 2026-02-17
**Status:** Draft â€” Fully reconciled with Custom Objects PRD
...
The Contact Management & Intelligence subsystem is the foundational entity layer of CRMExtender. Every other subsystem Ã¢â‚¬â€ conversations, deals, intelligence, relationship graphs Ã¢â‚¬â€ ultimately resolves back to contacts and companies. A communication has a sender and recipients. A deal has stakeholders. An intelligence item is about a person or organization. The contact subsystem answers the question: **"Who is this, and what do we know about them?"**

Unlike traditional CRMs that treat contacts as static address book entries, CRMExtender models contacts as **living intelligence objects** that evolve over time. Every field change is an immutable event. Every identifier Ã¢â‚¬â€ email, phone, social handle Ã¢â‚¬â€ has a lifecycle. Every data point carries source attribution and confidence scoring. The system doesn't just store who someone *is now*; it knows who they *were*, how their profile *changed*, and *why*.

**Core principles:**

- **Event-sourced contacts** Ã¢â‚¬â€ All contact and company mutations are stored as immutable events, enabling full audit trails, point-in-time reconstruction, and compliance (GDPR/CCPA data subject access requests).
- **Multi-identifier identity model** Ã¢â‚¬â€ A contact is not an email address. A person has multiple emails, phones, social handles, and aliases. The system resolves all of these to a single unified record.
- **Probabilistic entity resolution** Ã¢â‚¬â€ When data arrives from multiple sources (email sync, Google Contacts, browser extension, enrichment APIs, manual entry), the system determines whether two records refer to the same real-world person using tiered confidence scoring.
- **Intelligence-first** Ã¢â‚¬â€ Contacts are enriched automatically on creation. Behavioral signals, OSINT data, and communication patterns are continuously aggregated. AI generates briefings, suggests tags, and detects anomalies.
- **Temporal awareness** Ã¢â‚¬â€ Employment history, contact details, and relationship strengths are tracked over time with valid-from/valid-until bounds. The system knows that Sarah used to work at Acme Corp and now works at Globex, that her old email still resolves, and that her relationship with your team has strengthened over the last quarter.

**Current state:** The PoC implements a minimal contact model Ã¢â‚¬â€ a `contacts` table with `email`, `name`, `source`, and `source_id` columns populated from the Google People API. Contacts are used for conversation participant matching and triage filtering. This PRD defines the requirements for the full contact management system.

**Relationship to other PRDs:**

- **[Custom Objects PRD](custom-objects-prd.md)** â€” The Contact and Company entity types are **system object types** in the unified framework. Their table structure, field registry, event sourcing, and relation model are governed by the Custom Objects PRD. This PRD defines the Contact-specific behaviors (identity resolution, enrichment, intelligence scoring, relationship intelligence) that are registered with the object type framework. Custom fields on Contacts and Companies are managed through the unified field registry, not a JSONB column.
- **[Communication & Conversation Intelligence PRD](email-conversations-prd.md)** â€” Every communication participant resolves to a contact. The conversations subsystem depends on the contact model for participant resolution, but owns its own `conversation_participants` and `communication_participants` tables.
- **[Data Sources PRD](data-sources-prd.md)** â€” The Contact and Company virtual schema tables are derived from their object type field registries. The prefixed entity ID convention (`con_`, `cmp_`) enables automatic entity detection in data source queries.
- **[Data Layer Architecture PRD](data-layer-prd.md)** â€” Defines the `contacts` and `contact_identifiers` tables used by the conversations subsystem. This PRD extends the contact model with the full event-sourced schema, intelligence tables, and graph model.
...
Traditional CRM contact records are snapshots: a name, a company, an email, and a trail of manually logged activities. They degrade silently Ã¢â‚¬â€ people change jobs, companies rebrand, phone numbers rotate Ã¢â‚¬â€ and the CRM doesn't notice. The record you're looking at might be months or years out of date, and you have no way to know.
...
| **Siloed data sources**       | Contact intelligence exists across Google Contacts, LinkedIn, email signatures, enrichment APIs, and manual notes Ã¢â‚¬â€ but no system unifies them with proper attribution and conflict resolution.     |
...
- **Salesforce / HubSpot contacts** Ã¢â‚¬â€ Mutable record model. No event history, no point-in-time queries, no built-in enrichment, no graph-based relationships. Duplicates require third-party dedup tools. Custom field hell.
- **LinkedIn** Ã¢â‚¬â€ Rich profile data but locked behind their platform. No API access for most users. No integration with personal communication history. No custom intelligence.
- **Enrichment-only tools (Apollo, Clearbit, ZoomInfo)** Ã¢â‚¬â€ Provide data snapshots but no lifecycle tracking, no relationship modeling, no communication integration, no AI layer. Must be bolted onto a CRM.
- **Contact management apps (Google Contacts, Apple Contacts)** Ã¢â‚¬â€ Address book utilities. No intelligence, no relationships, no business context. Single-identifier model.
- **Graph-based CRMs (Affinity, Dex)** Ã¢â‚¬â€ Closer to the target, but limited enrichment, no event sourcing, no temporal history, no multi-channel communication integration.

CRMExtender closes this gap by making contacts **living intelligence objects** Ã¢â‚¬â€ event-sourced, multi-identifier, continuously enriched, graph-connected, and AI-analyzed.
...
1. **Unified contact identity** Ã¢â‚¬â€ Every person and organization encountered across any channel resolves to a single, canonical contact record via multi-identifier matching.
2. **Continuous intelligence** Ã¢â‚¬â€ Contacts are automatically enriched on creation and continuously monitored for changes. Stale data is detected and flagged.
3. **Temporal history** Ã¢â‚¬â€ Full event-sourced history enables point-in-time reconstruction, employment timelines, and audit compliance.
4. **Relationship modeling** Ã¢â‚¬â€ Contact-to-contact and contact-to-company relationships are modeled in a graph database with typed, temporal edges and strength scoring.
5. **AI-powered insights** Ã¢â‚¬â€ Every contact has an AI-generated briefing, suggested tags, engagement scoring, and anomaly detection available on demand.
6. **Zero-friction capture** Ã¢â‚¬â€ Contacts are created automatically from email participants, browser extension captures, and enrichment lookups. Manual entry is a last resort, not the default.
...
| ID    | Story                                                                                                                                                                       | Acceptance Criteria                                                                                                                               |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| CM-07 | As a user, I want to see a unified contact detail page showing all information about a person Ã¢â‚¬â€ profile, communication history, relationships, intelligence Ã¢â‚¬â€ in one place. | Contact detail page with sections: Profile, Timeline, Relationships, Intelligence, Employment History, Notes, Deals. All data loads within 200ms. |
...
| CM-09 | As a user, I want to see a contact's full employment history Ã¢â‚¬â€ every company, title, and department they've held Ã¢â‚¬â€ in chronological order.                                  | Employment history timeline component showing positions with start/end dates, sourced from event store and enrichment.                            |
...
| CM-21 | As a user, I want an AI-generated one-paragraph briefing on any contact that synthesizes all available data Ã¢â‚¬â€ profile, communication history, relationship context, recent intelligence. | "Brief me" action on contact detail. Claude generates briefing in < 5 seconds. Briefing cached with TTL. Refresh on demand.          |
...
| CM-23 | As a user, I want to see a visual network graph of a contact's relationships Ã¢â‚¬â€ who they know, who they work with, how they connect to my network.       | Interactive graph visualization centered on the selected contact. Zoom, pan, filter by relationship type. Click-through to related contacts. |
...
Contact (event-sourced)
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Identity: name, photo, aliases
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Classification: tags, categories, lead source, lead status
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Custom fields: managed through unified field registry (Custom Objects PRD Section 8)
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Computed: engagement_score, intelligence_score
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Identifiers (multi-value, lifecycle-tracked)
  Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Email addresses (work, personal, old)
  Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Phone numbers (mobile, work, home)
  Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Social profiles (LinkedIn, Twitter, GitHub)
  Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Any future identifier type
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Employment history (temporal)
  Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Company + Title + Department + Start/End
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Contact details (multi-value, temporal)
  Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Addresses (work, home)
  Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Key dates (birthday, anniversary)
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Intelligence items
  Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Enrichment data, OSINT, manual intel
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Relationships (graph edges)
  Ã¢â€â€š   Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Neo4j: KNOWS, REPORTS_TO, INTRODUCED_BY, etc.
  Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Activity timeline
      Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Communications, notes, deal events, intel updates
```

```
Company (event-sourced)
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Identity: name, domain, logo, industry
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Firmographics: size, location, revenue, funding
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ History: name changes, acquisitions, HQ moves
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Employees: contacts with WORKS_AT edges
  Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Intelligence items
```

```
Group (conventional table)
  Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Name, description, owner
  Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Members: many-to-many with contacts
```

### 5.2 Contact Record â€” Read Model Table (System Object Type)
...
| `company_id`         | TEXT | FK Ã¢â€ â€™ `companies(id)` | Current company (latest from `contacts__companies_employment` where `is_current=TRUE`)                                                                 |
...
| `engagement_score`   | REAL | DEFAULT 0.0                  | Composite engagement metric (0.0Ã¢â‚¬â€œ1.0). Computed from behavioral signals.                                                                   |
| `intelligence_score` | REAL | DEFAULT 0.0                  | Data completeness metric (0.0Ã¢â‚¬â€œ1.0). How much we know about this contact.                                                                   |
...
| ~~`custom_fields`~~  | â€”    | **REMOVED**          | ~~JSON object of tenant-defined custom field values.~~ **Removed:** Custom fields managed through the unified field registry (Custom Objects PRD Section 8). Each custom field is a typed column on the contacts table, added via `ALTER TABLE ADD COLUMN`.                                                                     |
| `created_by`         | TEXT | FK Ã¢â€ â€™ `users(id)`             | User who created the contact (NULL for auto-created)                                                                                       |
...
Multi-identifier resolution table. Every way a contact can be identified Ã¢â‚¬â€ email, phone, social handle, alias Ã¢â‚¬â€ is stored here with lifecycle tracking. This is the primary lookup table for resolving incoming communications to contacts.

| Column        | Type    | Constraints                                             | Description                                                                                                                          |
| ------------- | ------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `id`          | TEXT    | **PK**                                                  | UUID v4                                                                                                                              |
| `contact_id`  | TEXT    | NOT NULL, FK Ã¢â€ â€™ `contacts(id)` ON DELETE CASCADE | Which contact this identifier belongs to                                                                                             |
...
| `confidence`  | REAL    | DEFAULT 1.0                                             | Confidence that this identifier belongs to this contact (0.0Ã¢â‚¬â€œ1.0). Enrichment and auto-detection sources have < 1.0 confidence.      |
...
- `UNIQUE(type, value)` Ã¢â‚¬â€ No two contacts can claim the same identifier. If entity resolution discovers a conflict, it triggers a merge workflow.
...
| ------------- | ------- | ------------------------------------------------------- | ----------------------------------------- |
| `id`          | TEXT    | **PK**                                                  | UUID v4                                   |
| `contact_id`  | TEXT    | NOT NULL, FK Ã¢â€ â€™ `contacts(id)` ON DELETE CASCADE |                                           |
...
### 6.1 Company Record â€” Read Model Table (System Object Type)

> Company is a **system object type** (`is_system = true`, prefix `cmp_`). The `companies` table is its dedicated read model table within the tenant schema, managed through the object type framework.

| Column          | Type    | Constraints | Description                                                                                  |
| --------------- | ------- | ----------- | -------------------------------------------------------------------------------------------- |
| `id`            | TEXT    | **PK**      | Prefixed ULID: `cmp_` prefix (e.g., `cmp_01HX8A...`). Aligned with platform-wide convention.                                                                                      |
...
| ~~`custom_fields`~~ | â€”    | **REMOVED** | ~~JSON object of tenant-defined custom field values.~~ **Removed:** Custom fields managed through unified field registry.                       |
| `created_at`    | TEXT    | NOT NULL    | ISO 8601                                                                                     |
| `updated_at`    | TEXT    | NOT NULL    | ISO 8601                                                                                     |

### 6.2 Company Domain Resolution

Companies are often first encountered via email domain. The system maintains a mapping from email domains to companies:

```sql
-- When processing an email from sarah@acmecorp.com:
-- 1. Look up company by domain
SELECT id FROM companies WHERE domain = 'acmecorp.com';

-- 2. If not found, create a minimal company record
INSERT INTO companies (id, name, domain, created_at, updated_at)
VALUES (:id, 'acmecorp.com', 'acmecorp.com', :now, :now);
-- Note: name defaults to domain; enrichment will populate the real name.

-- 3. Link contact to company via employment junction table
INSERT INTO contacts__companies_employment (id, source_id, target_id, company_name, is_current, source, created_at)
VALUES (:id, :contact_id, :company_id, 'acmecorp.com', true, 'email_domain', :now);
```
...
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ email: sarah@acmecorp.com         (active, work, primary, verified, google_contacts)
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ email: sarah.chen@acmecorp.com    (active, work, verified, email_sync)
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ email: sarahc@gmail.com           (active, personal, verified, manual)
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ phone: +15550199                  (active, mobile, primary, enrichment_apollo)
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ linkedin: linkedin.com/in/sarahchen  (active, verified, linkedin_capture)
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Employment: VP Engineering @ Acme Corp (current, enrichment_apollo + linkedin_capture)
```
...
| **High**   | Name + Company + Title fuzzy match (>90% similarity) | 0.80Ã¢â‚¬â€œ0.95          | Auto-merge with review flag |
| **High**   | Name + Email domain match                            | 0.80Ã¢â‚¬â€œ0.90          | Auto-merge with review flag |
| **Medium** | Name + Company fuzzy match (no title)                | 0.60Ã¢â‚¬â€œ0.80          | Auto-merge with review flag |
| **Medium** | Name + Location match                                | 0.50Ã¢â‚¬â€œ0.70          | Queue for human review      |
| **Low**    | Name-only fuzzy match                                | 0.20Ã¢â‚¬â€œ0.50          | Queue for human review      |
...
Ã¢â€â€š  1. Identifier Extraction        Ã¢â€â€š
Ã¢â€â€š     Extract email, phone, name,  Ã¢â€â€š
Ã¢â€â€š     company, title, social URLs  Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               ...
               Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  2. Exact Match Lookup           Ã¢â€â€š
Ã¢â€â€š     Check contact_identifiers    Ã¢â€â€š
Ã¢â€â€š     for email, phone, LinkedIn   Ã¢â€â€š
Ã¢â€â€š     If found Ã¢â€ â€™ existing contact  Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š not found
               Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  3. Fuzzy Match Candidates       Ã¢â€â€š
Ã¢â€â€š     Name + Company similarity    Ã¢â€â€š
Ã¢â€â€š     via Meilisearch or trigram   Ã¢â€â€š
Ã¢â€â€š     Returns top-N candidates     Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š
               Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  4. Confidence Scoring           Ã¢â€â€š
Ã¢â€â€š     Score each candidate using   Ã¢â€â€š
Ã¢â€â€š     weighted signal combination  Ã¢â€â€š
Ã¢â€â€š     Apply tenant thresholds      Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š
     ...
     Ã¢â€“Â¼         Ã¢â€“Â¼          Ã¢â€“Â¼
  Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
  Ã¢â€â€š Auto Ã¢â€â€š Ã¢â€â€š Auto Ã¢â€â€š Ã¢â€â€š Human    Ã¢â€â€š
  Ã¢â€â€šMerge Ã¢â€â€š Ã¢â€â€šMerge Ã¢â€â€š Ã¢â€â€š Review   Ã¢â€â€š
  Ã¢â€â€š      Ã¢â€â€š Ã¢â€â€š+Flag Ã¢â€â€š Ã¢â€â€š Queue    Ã¢â€â€š
  Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
     Ã¢â€â€š         Ã¢â€â€š
     Ã¢â€“Â¼         Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  5. Merge Execution              Ã¢â€â€š
Ã¢â€â€š     - Pick survivor contact_id   Ã¢â€â€š
Ã¢â€â€š     - Move all identifiers       Ã¢â€â€š
Ã¢â€â€š     - Update participant FKs     Ã¢â€â€š
Ã¢â€â€š     - Emit MergeExecuted event   Ã¢â€â€š
Ã¢â€â€š     - Log in match_candidates    Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
```
...
| `confidence_score` | REAL | NOT NULL            | Combined confidence (0.0Ã¢â‚¬â€œ1.0)                                                                              |
...
| `reviewed_by`      | TEXT | FK Ã¢â€ â€™ `users(id)`    | User who reviewed (NULL for auto-merged)                                                                   |
| `reviewed_at`      | TEXT |                     | ISO 8601                                                                                                   |
| `created_at`       | TEXT | NOT NULL            | ISO 8601                                                                                                   |

**Constraints:**

- `UNIQUE(entity_type, entity_a_id, entity_b_id)` Ã¢â‚¬â€ One candidate record per pair.
...
| `captured_by` | TEXT | FK Ã¢â€ â€™ `users(id)` | User who captured (NULL for automated sources)                                                          |
...
1. **List selection** Ã¢â‚¬â€ Select 2+ contacts via checkboxes on the Contacts list, click "Merge Selected".
2. **Identifier conflict** Ã¢â‚¬â€ When adding an email in the Email Addresses section that belongs to another contact, the error message includes a "Merge these contacts?" link. (The generic Identifiers section has been removed from the contact detail page Ã¢â‚¬â€ email identifiers are managed via the type-specific Email Addresses section, backed by `contact_identifiers` where `type='email'`.)
...
5. **Post-merge domain resolution:** Resolve email domains from all merged identifiers and auto-create missing company affiliations via `domain_resolver`. This handles the common case where two contacts from different companies are merged Ã¢â‚¬â€ both affiliations are preserved.
...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------- |
| `incomplete` | Auto-created from an unknown identifier. Minimal data. Awaiting enrichment.                                                    | Ã¢â€ â€™ `active` (after enrichment or manual edit) |
| `active`     | Fully identified contact with at least name + one verified identifier.                                                         | Ã¢â€ â€™ `archived`, Ã¢â€ â€™ `merged`                     |
| `archived`   | User-archived contact. Excluded from active lists but data preserved. Identifiers still resolve for historical communications. | Ã¢â€ â€™ `active` (unarchive)                       |
| `merged`     | Duplicate contact that was merged into another record. Soft-deleted.                                                           | Ã¢â€ â€™ `active` (via split/undo merge)            |
...
Contact created / enrichment requested
  ...
  Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  Enrichment Dispatcher           Ã¢â€â€š
Ã¢â€â€š  (Celery task)                   Ã¢â€â€š
Ã¢â€â€š                                  Ã¢â€â€š
Ã¢â€â€š  Selects adapter based on:       Ã¢â€â€š
Ã¢â€â€š  - Available identifiers         Ã¢â€â€š
Ã¢â€â€š  - Adapter priority/cost         Ã¢â€â€š
Ã¢â€â€š  - Rate limits                   Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š
  ...
  Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
  Ã¢â€â€šApolloÃ¢â€â€š Ã¢â€â€šClear-Ã¢â€â€š Ã¢â€â€š  People  Ã¢â€â€š Ã¢â€â€š  Custom   Ã¢â€â€š
  Ã¢â€â€š      Ã¢â€â€š Ã¢â€â€š bit  Ã¢â€â€š Ã¢â€â€š Data LabsÃ¢â€â€š Ã¢â€â€š  Adapter  Ã¢â€â€š
  Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
     Ã¢â€â€š         Ã¢â€â€š          Ã¢â€â€š             Ã¢â€â€š
     Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¼Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€ËœÃ¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š
               Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  Enrichment Normalizer           Ã¢â€â€š
Ã¢â€â€š  - Maps adapter response to      Ã¢â€â€š
Ã¢â€â€š    common schema                 Ã¢â€â€š
Ã¢â€â€š  - Assigns confidence scores     Ã¢â€â€š
Ã¢â€â€š  - Detects conflicts with        Ã¢â€â€š
Ã¢â€â€š    existing data                 Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š
               Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  Enrichment Merger               Ã¢â€â€š
Ã¢â€â€š  - Higher-confidence data wins   Ã¢â€â€š
Ã¢â€â€š  - User-entered data always wins Ã¢â€â€š
Ã¢â€â€š  - Source attribution preserved  Ã¢â€â€š
Ã¢â€â€š  - Events emitted for all        Ã¢â€â€š
Ã¢â€â€š    changes                       Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š
               Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  Intelligence Score Update       Ã¢â€â€š
Ã¢â€â€š  - Recalculate intelligence_scoreÃ¢â€â€š
Ã¢â€â€š  - Update contacts table       Ã¢â€â€š
Ã¢â€â€š  - Index in Meilisearch          Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
```
...
| ------------- | ---- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id`          | TEXT | **PK**                       | UUID v4                                                                                                                                                  |
| `contact_id`  | TEXT | FK Ã¢â€ â€™ `contacts(id)`  | Linked contact (NULL if company-level intel)                                                                                                             |
| `company_id`  | TEXT | FK Ã¢â€ â€™ `companies(id)` | Linked company (NULL if contact-level intel)                                                                                                             |
...
| `confidence`  | REAL | DEFAULT 1.0                  | Confidence that this intel is accurate (0.0Ã¢â‚¬â€œ1.0)                                                                                                         |
| `verified_by` | TEXT | FK Ã¢â€ â€™ `users(id)`             | User who verified (NULL if unverified)                                                                                                                   |
...
|                  | `BOARD_MEMBER_OF`    | `since`, `until`                                     | Board membership (ContactÃ¢â€ â€™Company)        |
|                  | `INVESTOR_IN`        | `round`, `amount`                                    | Investment relationship (ContactÃ¢â€ â€™Company) |
...
| **Employment**   | `WORKS_AT`           | `role`, `department`, `since`, `until`, `is_current` | Employment (ContactÃ¢â€ â€™Company)              |
...
The `KNOWS` relationship includes a `strength` property (0.0Ã¢â‚¬â€œ1.0) that reflects the depth and recency of the relationship:
...
| ------------------------ | ----------------------------------------------------------------------------- | ----------------------------------------------------- |
| **Engagement score**     | Weighted composite of frequency, recency, and reciprocity across all channels | Overall relationship health (0.0Ã¢â‚¬â€œ1.0)                 |
...
| `owner_id`    | TEXT    | FK Ã¢â€ â€™ `users(id)` | User who created the group                                       |
...
| Column       | Type | Constraints                                             | Description |
| ------------ | ---- | ------------------------------------------------------- | ----------- |
| `group_id`   | TEXT | NOT NULL, FK Ã¢â€ â€™ `groups(id)` ON DELETE CASCADE           |             |
| `contact_id` | TEXT | NOT NULL, FK Ã¢â€ â€™ `contacts(id)` ON DELETE CASCADE |             |
| `added_at`   | TEXT | NOT NULL                                                | ISO 8601    |
| `added_by`   | TEXT | FK Ã¢â€ â€™ `users(id)`                                        |             |

**Constraints:** `PRIMARY KEY (group_id, contact_id)`

**Smart groups:** When `is_smart=1`, the group's membership is dynamically computed from `query_def` Ã¢â‚¬â€ a JSON-serialized filter (same format as saved views). Example:
...
| ------------ | ---- | ------------------------------------------------------- | ------------------------------------------------------ |
| `contact_id` | TEXT | NOT NULL, FK Ã¢â€ â€™ `contacts(id)` ON DELETE CASCADE |                                                        |
| `tag_id`     | TEXT | NOT NULL, FK Ã¢â€ â€™ `tags(id)` ON DELETE CASCADE             |                                                        |
...
- **Role tags:** Decision Maker, Technical, Executive, Legal, Finance Ã¢â‚¬â€ inferred from title and communication patterns.
- **Relationship tags:** VIP, Dormant, New Connection, Frequent Collaborator Ã¢â‚¬â€ based on engagement signals.
...
| **vCard 3.0/4.0**       | Apple Contacts, Outlook, Google (export) | Standard contact interchange. Multi-value fields (multiple emails/phones) supported. **Implemented** Ã¢â‚¬â€ `poc/vcard_import.py`, web UI at `/contacts/import`, CLI `import-vcards`. Three-tier duplicate detection: email match Ã¢â€ â€™ normalized phone match Ã¢â€ â€™ exact name match (customer-scoped). |
...
File uploaded / API sync initiated
  ...
  Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  1. Parse & Validate             Ã¢â€â€š
Ã¢â€â€š     - Parse file format          Ã¢â€â€š
Ã¢â€â€š     - Validate required fields   Ã¢â€â€š
Ã¢â€â€š     - Normalize values           Ã¢â€â€š
Ã¢â€â€š     - Report parsing errors      Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š
               Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  2. Duplicate Detection          Ã¢â€â€š
Ã¢â€â€š     - Check each record against  Ã¢â€â€š
Ã¢â€â€š       existing contacts via      Ã¢â€â€š
Ã¢â€â€š       entity resolution pipeline Ã¢â€â€š
Ã¢â€â€š     - Classify: new, duplicate,  Ã¢â€â€š
Ã¢â€â€š       update                     Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š
               Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  3. Preview & User Confirmation  Ã¢â€â€š
Ã¢â€â€š     - Show import preview:       Ã¢â€â€š
Ã¢â€â€š       N new, N updates, N dupes  Ã¢â€â€š
Ã¢â€â€š     - User selects handling for  Ã¢â€â€š
Ã¢â€â€š       each category              Ã¢â€â€š
Ã¢â€â€š     - User confirms import       Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š
               Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  4. Execute Import               Ã¢â€â€š
Ã¢â€â€š     - Create new contacts        Ã¢â€â€š
Ã¢â€â€š     - Merge updates into existingÃ¢â€â€š
Ã¢â€â€š     - Skip or merge duplicates   Ã¢â€â€š
Ã¢â€â€š     - Emit events for all changesÃ¢â€â€š
Ã¢â€â€š     - Trigger enrichment for new Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
               Ã¢â€â€š
               Ã¢â€“Â¼
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š  5. Import Report                Ã¢â€â€š
Ã¢â€â€š     - Summary: created, updated, Ã¢â€â€š
Ã¢â€â€š       skipped, errored           Ã¢â€â€š
Ã¢â€â€š     - Error details for failed   Ã¢â€â€š
Ã¢â€â€š       records                    Ã¢â€â€š
Ã¢â€â€š     - Import ID for audit trail  Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
```
...
You've exchanged 47 emails over the last 6 months Ã¢â‚¬â€ your relationship is active and
...
| Warm intro opportunity            | "Sarah knows Marcus Rodriguez at TechVentures Ã¢â‚¬â€ they worked together at Globex." |
...
| `actor_id`        | TEXT    | FK Ã¢â€ â€™ `users(id)` | User who triggered the event (NULL for system events) |
| `timestamp`       | TIMESTAMPTZ  | NOT NULL         |                                              |
| `sequence_number` | INTEGER | NOT NULL         | Monotonically increasing per entity, for ordering     |

**Constraints:**

- `UNIQUE(entity_id, sequence_number)` Ã¢â‚¬â€ guarantees event ordering per entity.
...
| `IdentifierStatusChanged` | `type`, `value`, `old_status`, `new_status`         | Active Ã¢â€ â€™ inactive, etc.           |
...
- `contacts` Ã¢â‚¬â€ Full contact records for the user's accessible contacts.
- `contact_identifiers` Ã¢â‚¬â€ For local search and communication participant resolution.
- `contacts__companies_employment` Ã¢â‚¬â€ For contact detail display.
- `companies` Ã¢â‚¬â€ Company records referenced by contacts.
- `tags` and `contact_tags` Ã¢â‚¬â€ For filtering.
- `groups` and `group_members` Ã¢â‚¬â€ For group membership display.

**Not synced to client (server-only):**

- Event store Ã¢â‚¬â€ Too large, not needed for display.
- Intelligence items Ã¢â‚¬â€ Fetched on demand via API.
- Graph relationships Ã¢â‚¬â€ Queried live from Neo4j via API.
- Match candidates Ã¢â‚¬â€ Admin/review workflow is server-side only.
...
| ---------------------- | --------------------------------------------------------------------- |
| **Right to know**      | Same as GDPR right of access Ã¢â‚¬â€ full data export.                      |
| **Right to delete**    | Same as GDPR right to erasure Ã¢â‚¬â€ hard delete workflow.                 |
...
### Phase 2 Ã¢â‚¬â€ Intelligence Engine
...
### Phase 3 Ã¢â‚¬â€ Relationship Intelligence
...
| ~~Custom field builder~~  | ~~P0~~   | ~~Tenant-configurable typed fields~~ **Resolved:** Custom fields managed through unified field registry (Custom Objects PRD Section 8). Standard field management UI for any object type. No separate builder needed â€” it's the standard field management UI for any object type.                          |
...
| **Apollo**                  | REST API             | Enrichment Ã¢â€ â€™ contact updates         | Phase 2 |
| **Clearbit**                | REST API             | Enrichment Ã¢â€ â€™ contact updates         | Phase 2 |
| **People Data Labs**        | REST API             | Enrichment Ã¢â€ â€™ contact updates         | Phase 2 |
| **LinkedIn (browser ext.)** | Extension Ã¢â€ â€™ API      | Profile capture Ã¢â€ â€™ contacts           | Phase 2 |
| **Twitter (browser ext.)**  | Extension Ã¢â€ â€™ API      | Profile capture Ã¢â€ â€™ contacts           | Phase 2 |
...
1. **Contact ownership model** Ã¢â‚¬â€ Should contacts be owned by individual users, shared across the team, or both (personal vs. shared pools)? Impacts visibility, editing rights, and dedup scope.
...
3. **Enrichment budget management** Ã¢â‚¬â€ Should enrichment API usage have per-tenant monthly budgets? Per-contact caps? How to prioritize enrichment when budget is constrained?

4. **Graph relationship creation** Ã¢â‚¬â€ How much relationship creation should be automated (inferred from communication patterns) vs. manual? Automated inference risks false relationships.

5. **Contact photo storage** Ã¢â‚¬â€ Store photos in object storage or link to external URLs (Gravatar, LinkedIn)? External URLs break if source changes; local storage costs more.

6. **Multi-tenant contact sharing** Ã¢â‚¬â€ For users who belong to multiple tenants, should contacts be shareable across tenants? Impacts data isolation model.

7. **Contact scoring customization** Ã¢â‚¬â€ Should tenants be able to customize engagement and intelligence score weights? Adds complexity but increases relevance per use case.

8. **LinkedIn API partnership** Ã¢â‚¬â€ LinkedIn's API is restricted. Evaluate Voyager API access, LinkedIn Sales Navigator integration, or browser extension only.

9. **Email signature parsing accuracy** Ã¢â‚¬â€ Email signatures contain valuable contact data (name, title, phone, address) but parsing is error-prone. Evaluate ML-based signature parsing vs. regex vs. third-party service.

10. **Historical enrichment** Ã¢â‚¬â€ When enrichment data arrives, should it retroactively update intelligence items and scores for historical periods, or only apply going forward?
...
| **Intelligence score** | Composite metric (0.0Ã¢â‚¬â€œ1.0) reflecting how much the system knows about a contact                               |
| **Engagement score**   | Composite metric (0.0Ã¢â‚¬â€œ1.0) reflecting the health and recency of the relationship                              |
| **Warm intro path**    | A chain of mutual connections between the user and a target contact                                           |
| **OSINT**              | Open-Source Intelligence Ã¢â‚¬â€ publicly available data about people and companies                                 |
| **DSAR**               | Data Subject Access Request Ã¢â‚¬â€ a GDPR right for individuals to request their data                              |
