...
Object Type Framework
  ...
  │     ├── Field Groups: logical sections for detail view rendering
...
| **Field Group** | A named grouping of fields for organizing the detail view layout. Purely presentational — does not affect storage or queries. |
...
| `field_group_id` | TEXT | FK → field_groups | Optional grouping for detail view layout. See Section 11. |
...
Field groups organize fields into logical sections on the record detail view. They are purely presentational — they do not affect physical storage, query behavior, or field ordering in views.
...
- Use case: a Communication references a Conversation (the Conversation doesn't need a "Communications" field — its activity timeline handles that differently)
