...
- **[Contact Management PRD](contact-management-prd_V4.md)** — Contacts participate in events through the Event→Contact Relation Type. Calendar sync resolves attendee emails to contact records via `contact_identifiers`. Event co-attendance feeds relationship strength scoring alongside email co-occurrence.
...
- **[Views & Grid PRD](views-grid-prd_V3.md)** — Event views, filters, sorts, and inline editing operate on fields defined in the Event field registry. Calendar View and Timeline View types are natural fits for event data.
- **[Permissions & Sharing PRD](permissions-sharing-prd_V1.md)** — Event record access, sync permissions, and calendar integration management follow the standard role-based access model.
...
Event fields are organized into field groups for the record detail view:
...
| **[Views & Grid PRD](views-grid-prd_V3.md)** | Event views, filters, sorts use fields from the Event field registry. Calendar View and Timeline View are natural fits. | **Views depend on Events** for field definitions. |
| **[Permissions & Sharing PRD](permissions-sharing-prd_V1.md)** | Event record access, calendar sync permissions, integration management. | **Events depend on Permissions** for access control. |
