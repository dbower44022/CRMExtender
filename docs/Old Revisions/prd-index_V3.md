...
| ----------------------------------------- | ------- | ---------- | ----------------------- | ------ | --------- |
| Communication & Conversation Intelligence | 2.0     | 2026-02-07 | Draft â€” Complete      | ~2,800 | Dedicated |
| Contact Management & Intelligence         | 1.0     | 2026-02-08 | Draft â€” Complete      | ~4,000 | Dedicated |
| Views & Grid System                       | 1.1     | 2026-02-16 | Draft â€” Complete      | ~1,600 | Dedicated |
| Data Sources                              | 1.0     | 2026-02-16 | Draft â€” Complete      | ~1,500 | Dedicated |
| Custom Objects                            | 1.0     | 2026-02-17 | Draft â€” Complete      | ~1,500 | Dedicated |
| Email Parsing & Content Extraction        | â€”     | â€”        | Technical Spec          | ~700   | â€”       |
| AI Learning & Classification              | â€”     | â€”        | **Not Started**         | â€”    | â€”       |
| Permissions & Sharing                     | â€”     | â€”        | **Not Started**         | â€”    | â€”       |
| CRMExtender Platform (Parent)             | 1.1     | â€”        | Exists (not in project) | â€”    | â€”       |
...
**Scope:** The primary relationship signal layer. Captures communications across email, SMS, phone, video, in-person, and notes. Organizes them into a Project â†’ Topic â†’ Conversation â†’ Communication hierarchy.
...
- Contacts use UUID v4 (**migration to prefixed ULID queued** â€” see Reconciliation Items)
- Employment history tracked as separate table with temporal records (**reframe as relation type with metadata queued** â€” see Reconciliation Items)
...
- Remove `custom_fields` JSONB column â€” custom fields on Contacts managed through unified field registry
...
- Reframe `employment_history` as a system relation type (Contactâ†’Company) with temporal metadata (start_date, end_date, title, department)
...
**File:** `views-grid-prd_V2.md` â†’ updated to `views-grid-prd_V3.md`
...
- Data Sources (summary â€” full spec in Data Sources PRD)
...
- **Full event sourcing for all entity types** â€” per-entity-type event tables (e.g., `jobs_events`). Same audit trail and point-in-time reconstruction for custom and system entities.
- **First-class Relation Types** â€” all three cardinalities (1:1, 1:many, many:many) from Phase 1. Bidirectional or unidirectional per relation type. Self-referential supported. Configurable cascade (nullify default, restrict, cascade archive).
- **Relation metadata** â€” additional attributes on relationship instances (role, strength, start date, notes). Stored on junction tables or companion instance tables.
- **Neo4j graph sync** â€” optional flag per relation type. Relation instances synced as graph edges. System relations synced Phase 1; custom relation sync Phase 2.
...
- **Limited safe field type conversions** â€” conversion matrix with preview wizard. Immutable slug/prefix.
...
**Gutter business test cases:** Jobs, Properties, Service Areas, Estimates â€” used as running examples throughout the PRD.
...
â”‚  Communication      â”‚ â”‚  Contact            â”‚ â”‚  Custom Objects     â”‚
â”‚  & Conversation     â”‚ â”‚  Management &       â”‚ â”‚  â˜… FOUNDATION       â”‚
â”‚  Intelligence       â”‚ â”‚  Intelligence       â”‚ â”‚                     â”‚
â”‚                     â”‚ â”‚                     â”‚ â”‚  Entity types,      â”‚
â”‚  System entity      â”‚ â”‚  System entity      â”‚ â”‚  field registry,    â”‚
â”‚  types: Conversationâ”‚ â”‚  types: Contact,    â”‚ â”‚  relations, storage,â”‚
â”‚  Communication,     â”‚ â”‚  Company            â”‚ â”‚  event sourcing     â”‚
â”‚  Project, Topic     â”‚ â”‚                     â”‚ â”‚                     â”‚
...
â”‚  Data Sources        â”‚â—„â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
â”‚                      â”‚         â”‚   Virtual schema from object type
â”‚  Virtual schema,     â”‚         â”‚   field registries, relation model
â”‚  query engine,       â”‚         â”‚
â”‚  column registries   â”‚         â”‚
...
â”‚  System              â”‚         â”‚
â”‚                      â”‚         â”‚
â”‚  Field rendering,    â”‚         â”‚
â”‚  relation traversal  â”‚         â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜         â”‚
         â”‚                       â”‚
         â”‚    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
         â–¼    â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”         â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  Permissions &       â”‚         â”‚  AI Learning &       â”‚
â”‚  Sharing             â”‚         â”‚  Classification      â”‚
â”‚  (PLANNED â€” #1       â”‚         â”‚  (PLANNED â€” #2       â”‚
...
**â˜… Custom Objects as foundation:** The Custom Objects PRD defines the entity model that Data Sources, Views, Contact Management, and Communication all build upon. It is the most cross-cutting completed PRD â€” all entity types (system and custom) are defined through its framework.
...
| ------------------------------ | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Prefixed entity IDs**        | Data Sources, Custom Objects, Contact Management                | **Resolved by Custom Objects PRD.** Section 6.2â€“6.3 establishes type prefix registry, generation algorithm, and collision handling. All entity types use `{prefix}_{ULID}` format. Contact Management PRD update queued to migrate from UUID v4 to `con_` prefixed ULIDs. |
...
| **Custom entity storage**      | Data Sources, Custom Objects                                    | **Resolved by Custom Objects PRD.** Dedicated typed tables per entity type with DDL-at-runtime (Section 17â€“18). Virtual schema maps 1:1 to physical schema (Section 17.3).                                                                                                |
...
| -------------------------------------------------- | ------------------------------------------------ | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~**Contact Management `custom_fields` removal**~~ | Contact Management, Custom Objects               | **Completed in V3** | `custom_fields` JSONB removed. Custom fields managed through unified field registry.                                                                                                                                                   |
| ~~**Contact entity ID migration**~~                | Contact Management, Custom Objects, Data Sources | **Completed in V2** | Contact IDs use `con_` prefixed ULIDs. Company IDs use `cmp_` prefixed ULIDs.                                                                                                                                                          |
| ~~**Employment history → Relation Type**~~         | Contact Management, Custom Objects               | **Completed in V3** | Employment history fully rewritten as system Relation Type (Contact→Company, many-to-many) with junction table `contacts__companies_employment`.                                                                                       |
| ~~**`contacts_current` → Object Type table**~~     | Contact Management, Custom Objects               | **Completed in V3** | All `contacts_current`/`companies_current` references replaced with `contacts`/`companies`. Event store migrated to per-entity-type tables. Merge flow reconciled.                                                                     |
| **Communication entities â†’ System Object Types** | Communication, Custom Objects                    | **Queued**          | Conversation, Communication, Project, Topic entities need reframing as system object types in the unified framework. Tables created by tenant schema provisioning. Specialized behaviors registered per Custom Objects PRD Section 22. |
...
1. **Permissions & Sharing** â€” Most cross-cutting remaining dependency. Every PRD references it. Custom Objects PRD provides specific inputs: Object Creator permission, field-level permissions (Phase 4), schema-per-tenant architecture, record-level access control for all entity types.
2. **AI Learning & Classification** â€” Depends on Communication PRD signals, Contact Management context, and Custom Objects entity type awareness. Can be written once the permission model is stable.
3. ~~**Contact Management PRD v2**~~ **Done (V3).** Contact Management PRD fully reconciled with Custom Objects unified framework.
