...
|---|---|---|---|---|
| Custom Objects | 1.0 | `custom-objects-prd.md` | Draft â€” Foundation PRD | 2026-02-17 |
| Communications | 1.0 | `communications-prd_V1.md` | Draft â€” Reconciled with Custom Objects | 2026-02-18 |
| Conversations | 1.0 | `conversations-prd_V1.md` | Draft â€” Reconciled with Custom Objects | 2026-02-18 |
| Contact Management | 4.0 | `contact-management-prd_V4.md` | Draft â€” Reconciled with Custom Objects | 2026-02-17 |
| Company Management | 1.0 | `company-management-prd.md` | Draft â€” Needs V2 reconciliation | 2026-02-09 |
| Event Management | 2.0 | `events-prd_V2.md` | Draft â€” Reconciled with Custom Objects | 2026-02-17 |
| Notes | 2.0 | `notes-prd_V2.md` | Draft â€” Reconciled with Custom Objects | 2026-02-17 |
| Tasks | 1.0 | `tasks-prd_V1.md` | Draft — Reconciled with Custom Objects | 2026-02-18 |
| Views & Grid | 3.0 | `views-grid-prd_V3.md` | Draft â€” Complete | 2026-02-15 |
| Data Sources | 1.0 | `data-sources-prd.md` | Draft â€” Complete | 2026-02-14 |
| Permissions & Sharing | 1.0 | `permissions-sharing-prd_V1.md` | Draft â€” Complete | 2026-02-16 |
| Email Parsing & Content Extraction | â€” | `email_stripping.md` | Technical spec | 2026-02-07 |
| Email Provider Sync | â€” | â€” | **Planned** â€” parent (Communications) complete | â€” |
| SMS/MMS | â€” | â€” | **Planned** â€” parent (Communications) complete | â€” |
| Voice/VoIP | â€” | â€” | **Planned** â€” parent (Communications) complete | â€” |
| Video Meetings | â€” | â€” | **Planned** â€” parent (Communications) complete | â€” |
| AI Learning & Classification | â€” | â€” | **Planned** â€” requirements established in Conversations PRD | â€” |
| Contact Intelligence | â€” | â€” | **Planned** â€” depends on Contact Mgmt + Communications | â€” |
...
- **Unified Object Model** â€” system entities are pre-installed object types with `is_system` flag, protected core fields, and registered behaviors. Custom entities are equal citizens at the storage, query, and rendering layers.
- **Dedicated typed tables per entity type** â€” every entity type gets its own PostgreSQL table with native typed columns. Fields map to columns. DDL at runtime via ALTER TABLE.
- **Schema-per-tenant** â€” each tenant gets its own PostgreSQL schema with entity tables, event tables, and junction tables.
- **Full event sourcing for all entity types** â€” per-entity-type event tables. Same audit trail and point-in-time reconstruction for custom and system entities.
- **First-class Relation Types** â€” all three cardinalities (1:1, 1:many, many:many) from Phase 1. Bidirectional or unidirectional. Self-referential supported. Configurable cascade (nullify default, restrict, cascade archive).
- **Relation metadata** â€” additional attributes on relationship instances (role, strength, start date, notes).
- **Neo4j graph sync** â€” optional flag per relation type.
- **Permission-gated entity type creation** â€” Object Creator permission (not admin-only).
...
- Communication Participants Relation Type (Communicationâ†’Contact, many-to-many with role/address metadata)
...
- `conversation_id` as FK column (not Relation Type) â€” simpler for strict many:1
...
|---|---|---|
| Email Provider Sync PRD | Gmail, Outlook, IMAP adapters; dual-track parsing; email threading; email-specific triage | Planned â€” parent complete |
...
- Organizational hierarchy (Project â†’ Sub-project â†’ Topic â†’ Conversation â†’ Communication)
...
- User-defined alerts (no defaults, viewâ†’alert architecture, frequency/aggregation/delivery)
- Event sourcing for all three entity types
- API design (CRUD for conversations/topics/projects, intelligence, review workflow)

**Key decisions made:**

- Three separate system object types (Conversation, Topic, Project) â€” not one hierarchy entity
- Derived participant list (no Conversationâ†’Contact Relation Type â€” avoids duplication)
- Optional hierarchy at every level â€” communications can be unassigned, conversations don't need topics
...
- No default alerts â€” user defines all notifications
...
- Employment history as system Relation Type (Contactâ†’Company, many-to-many with temporal metadata)
...
- Engagement score is a composite behavioral metric (0.0â€“1.0)
- Intelligence score measures data completeness (0.0â€“1.0)
...
- Eventâ†”Contact Relation Type (`event_attendees`)
- Eventâ†”Conversation Relation Type (`event_conversations`)
...
- Shared views enforce row-level security (sharing definition â‰  sharing data)
- Board view supports swimlanes (matrix of group-by Ã— status)
...
- Visual query builder (5-step: entity â†’ joins â†’ columns â†’ filters â†’ sort)
...
- Entity detection & preview system (3-layer: auto-detect â†’ inference rules â†’ manual override)
- Data source â†’ view relationship (many-to-one, composition rules)
- Inline editing trace-back (column registry â†’ source entity â†’ API call)
...
- Prefixed entity IDs (`con_`, `cvr_`, `com_`, etc.) with ULID â€” adopted platform-wide
...
**Alignment with Custom Objects PRD:** Virtual schema composition resolved â€” virtual schema tables = object type slugs, virtual schema columns = field slugs. 1:1 mapping means query engine translation is trivial.
...
- Shared view permissions (sharing definition â‰  sharing data)
...
### 12. Email Parsing & Content Extraction
...
**Parent:** Communications PRD (now complete â€” defines the provider adapter interface and common schema)
...
| Data Source | `dts_` | `data_sources` | Data Sources PRD | â€” |
| View | `viw_` | `views` | Views & Grid PRD | â€” |
| User | `usr_` | `users` | Permissions & Sharing PRD | â€” |
| Segment | `seg_` | `segments` | Contact Management PRD | â€” |
...
â”‚  Custom Objects     â”‚ â”‚  Contact            â”‚ â”‚  Permissions &      â”‚
â”‚  â˜… FOUNDATION       â”‚ â”‚  Management         â”‚ â”‚  Sharing            â”‚
â”‚                     â”‚ â”‚                     â”‚ â”‚                     â”‚
â”‚  Entity types,      â”‚ â”‚  System entity      â”‚ â”‚  Roles, RBAC,       â”‚
â”‚  field registry,    â”‚ â”‚  types: Contact,    â”‚ â”‚  row-level security,â”‚
â”‚  relations, storage,â”‚ â”‚  Company            â”‚ â”‚  provider accounts  â”‚
â”‚  event sourcing     â”‚ â”‚                     â”‚ â”‚                     â”‚
...
â”‚  Communications     â”‚  â”‚  Conversations      â”‚
â”‚  PRD                â”‚  â”‚  PRD                â”‚
â”‚                     â”‚  â”‚                     â”‚
â”‚  Communication      â”‚â”€â”€â”‚  Conversation (cvr_)â”‚
â”‚  entity (com_)      â”‚  â”‚  Topic (top_)       â”‚
â”‚  Provider adapters  â”‚  â”‚  Project (prj_)     â”‚
â”‚  Triage pipeline    â”‚  â”‚  AI intelligence    â”‚
â”‚  Contact resolution â”‚  â”‚  Hierarchy model    â”‚
â”‚  Attachments        â”‚  â”‚  Review workflow    â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜  â”‚  Views & alerts     â”‚
        â”‚                â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
   â”Œâ”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
   â”‚    Channel Child PRDs       â”‚
   â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
   â”‚ Email Provider Sync (planned)â”‚
   â”‚ SMS/MMS (planned)           â”‚
   â”‚ Voice/VoIP (planned)        â”‚
   â”‚ Video Meetings (planned)    â”‚
   â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜

Other system object types:
+----------------+  +----------------+  +----------------+
|  Event Mgmt    |  |  Notes PRD     |  |  Tasks PRD     |
|  PRD (evt_)    |  |  (not_)        |  |  (tsk_)        |
+----------------+  +----------------+  +----------------+
...
â”‚  Views &     â”‚  â”‚  Data        â”‚
â”‚  Grid PRD    â”‚  â”‚  Sources PRD â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```

**Reading the arrows:** An arrow from A to B means "A depends on B" or "A references B."

**â˜… Custom Objects as foundation:** The Custom Objects PRD defines the entity model that Data Sources, Views, Contact Management, Communications, Conversations, Events, Notes, and Tasks all build upon. All entity types (system and custom) are defined through its framework.
...
|---|---|---|
| **Prefixed entity IDs** | Data Sources, Custom Objects, all entity PRDs | Resolved by Custom Objects PRD Section 6.2â€“6.3. All entity types use `{prefix}_{ULID}` format. |
...
| **Custom entity storage** | Data Sources, Custom Objects | Resolved by Custom Objects PRD. Dedicated typed tables with DDL-at-runtime (Section 17â€“18). |
...
| **Conversation participants** | Conversations PRD, Communications PRD | Resolved in Conversations PRD Section 5.5. Derived from Communication Participants (no separate Conversationâ†’Contact relation). |
...
| Employment history â†’ Relation Type | Contact Management V3 | 2026-02-17 | Junction table with temporal metadata |
| `contacts_current` â†’ object type table | Contact Management V3 | 2026-02-17 | `contacts` read model managed by framework |
| Events â†’ system object type | Events V2 | 2026-02-17 | Full rewrite from PoC SQLite |
| Notes â†’ system object type | Notes V2 | 2026-02-17 | Universal Attachment Relation pattern |
| Communication entities â†’ system object types | Communications V1, Conversations V1 | 2026-02-18 | Full decomposition + Custom Objects reconciliation |
...
1. **Company Management V2** â€” Reconcile with Custom Objects framework (prefixed ULIDs, field registry, relation types, event sourcing). Similar effort to Events V2.
2. **Email Provider Sync PRD** â€” First channel child PRD. Gmail, Outlook, IMAP adapters; dual-track parsing pipeline; email-specific triage. Parent (Communications PRD) is complete.
3. **AI Learning & Classification PRD** â€” Requirements established by Conversations PRD. Depends on Communications (triage correction signals) and Conversations (assignment correction signals).
4. **SMS/MMS PRD** â€” Second channel child PRD. Provider selection still open (Twilio vs. OpenPhone).
...
| `prd-index_V5.md` | This file (replaces V4) |
