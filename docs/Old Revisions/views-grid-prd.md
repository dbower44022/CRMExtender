...
6. [View Types](#6-view-types)
7. [The Column System](#7-the-column-system)
8. [Field Type Registry](#8-field-type-registry)
9. [Relation Traversal & Lookup Columns](#9-relation-traversal--lookup-columns)
10. [Filtering & Query Builder](#10-filtering--query-builder)
11. [Sorting & Grouping](#11-sorting--grouping)
12. [Grid Interactions](#12-grid-interactions)
13. [Calendar View Specifics](#13-calendar-view-specifics)
14. [Board View Specifics](#14-board-view-specifics)
15. [Timeline View Specifics](#15-timeline-view-specifics)
16. [View Persistence & Sharing](#16-view-persistence--sharing)
17. [View-as-Alert Integration](#17-view-as-alert-integration)
18. [Performance & Pagination](#18-performance--pagination)
...
```
Entity Type (from Object Model)
  ├── Field Registry (system fields + custom fields)
  │     ├── Field: Name (text)
  │     ├── Field: Status (select)
  │     ├── Field: Created At (datetime)
  │     ├── Field: Related Contact (relation → Contact)
  │     └── Field: Custom Score (number, user-defined)
  │
  └── Views (user-created lenses)
        ├── View: "My Active Deals" (List/Grid)
        │     ├── Columns: [Name, Status, Contact→Company, Last Activity, Custom Score]
        │     ├── Filters: Status = Active AND Custom Score > 50
        │     ├── Sort: Last Activity DESC
        │     └── Groups: none
        │
        ├── View: "Deal Timeline" (Timeline)
        │     ├── Date Range: Start Date → Expected Close
        │     ├── Filters: Status != Closed
        │     └── Groups: Owner
        │
        ├── View: "Deal Pipeline" (Board)
        │     ├── Grouping Field: Deal Stage
        │     ├── Card Fields: [Name, Value, Contact→Company]
        │     ├── Filters: Owner = Me
        │     └── Swimlanes: none
        │
        └── View: "Follow-Up Calendar" (Calendar)
              ├── Date Field: Next Follow-Up Date
              └── Filters: Assigned to Me
```
...
| Concept | Definition |
|---|---|
| **Entity Type** | A class of objects in the system. Can be system-defined (Contact, Conversation, Communication, Project, Topic) or user-defined (via the Custom Objects system). Each entity type has a field registry. |
...
| **View** | A named, saved configuration that defines how to display a set of records of a specific entity type. Includes: view type, visible columns (with order, width, pinning), filter conditions, sort orders, and group-by configuration. |
| **View Type** | The rendering strategy for a view: List/Grid (tabular rows and columns), Calendar (records plotted on a date grid), or Timeline (records as horizontal bars across a time axis). |
...
┌──────────────────────────────────────────────────────────────────┐
│                        VIEW CONFIGURATION                         │
│  (columns, filters, sorts, groups, view type, entity type)        │
└──────────────────────┬───────────────────────────────────────────┘
                       │
                       ▼
┌──────────────────────────────────────────────────────────────────┐
│                         QUERY ENGINE                              │
│  Translates view config into data queries:                        │
│  - Reads field registry from Object Model                         │
│  - Resolves relation traversals to JOINs                          │
│  - Applies filters, sorts, groups                                 │
...
### 6.5 View Type Compatibility Matrix
...
### 8.1 Field Types and View Behavior
...
### 8.2 Field Types and Filter Operators
...
## 9. Relation Traversal & Lookup Columns
...
### 9.4 Relation Traversal in the Column Picker
...
### 12.2 Row Expansion / Detail Panel
...
## 13. Calendar View Specifics

### 13.1 Calendar Rendering Model
...
### 13.3 Calendar-Specific Interactions
...
## 14. Board View Specifics

### 14.1 Board Rendering Model
...
### 14.3 Board-Specific Interactions
...
### 14.4 Drag-to-Column Confirmation
...
## 15. Timeline View Specifics

### 15.1 Timeline Rendering Model
...
### 15.3 Timeline-Specific Interactions
...
## 16. View Persistence & Sharing

### 16.1 View Data Model
...
| **Entity type** | The entity type this view displays (e.g., Conversation, Contact, custom "Jobs" object) |
| **View type** | List, Calendar, or Timeline |
| **Owner** | The user who created the view |
| **Visibility** | `personal` (only the owner can see it) or `shared` (visible to the team/workspace) |
| **Is default** | Whether this is the default view for its entity type for the owner |
| **Column configuration** | Ordered list of columns with field mapping, width, pinned state, visibility, label override |
| **Filter configuration** | Filter tree (conditions + groups with AND/OR logic) |
| **Sort configuration** | Ordered list of sort rules (field + direction) |
...
| Aspect | Behavior |
|---|---|
| **Sharing** | The view owner sets visibility to `shared`. The view appears in the team's view list for that entity type. |
| **Data permissions** | Sharing a view shares the query definition, not data access. Each viewer sees only records they have permission to view. A shared "All Conversations" view shows different results for different users based on their data permissions. |
...
### 17.3 Alert-View Synchronization
...
|---|---|---|
| **View configuration** | Column, filter, sort, group settings | Until modified |
| **Field registry** | Entity type field definitions | Until schema change |
| **Query results** | Current page of records | Until data mutation or 60 seconds (whichever is first) |
...
### Phase 1: List/Grid Foundation

**Goal:** Deliver a functional, entity-agnostic List/Grid view for system entities.

- View CRUD (create, read, update, delete)
- Column system: direct field columns, add/remove/reorder/resize/pin
...
- Basic inline editing for text, number, select, checkbox, date fields
...
### Phase 2: Advanced List/Grid + Sharing

**Goal:** Full filter power, grouping, bulk actions, shared views.
...
### Phase 3: Relation Traversal + Calendar + Board Views

**Goal:** Cross-entity context in grids; Calendar and Board view types.

- Relation traversal columns (one-hop lookups)
- Relation-field filtering
- Relation-field sorting
- Column picker with relation traversal discovery UI
...
|---|---|---|
| **Custom Objects PRD** | Defines the entity types, field registry, field types, and relation model that the view system renders. The view system is entity-agnostic — it reads from whatever schema the object model provides. | **View system depends on Object Model.** The view system cannot function without a field registry. |
| **Communication & Conversation Intelligence PRD** | Defines system entities (Conversations, Communications, Projects, Topics, Contacts) that are the primary data sources for views. Also defines the alert system architecture that views feed into. | **Bidirectional.** Views render Conversation data; Conversations PRD's alert system consumes view filter definitions. |
| **Contact Intelligence PRD** | Defines the Contact entity and identity resolution system. Contact-related views (and relation traversal to Contacts from other entities) depend on this. | **View system depends on Contact model** for Contact entity fields and relation resolution. |
| **Permissions & Sharing PRD** | Defines data access controls that determine which records a user can see in a view. Shared view visibility is governed by these permissions. | **View system depends on Permissions** for data filtering and shared view access control. |
| **AI Learning & Classification PRD** | AI-generated fields (AI Status, AI Summary, Action Items, Key Topics) are displayed as columns in views. The AI system may also use view configurations as context for learning user preferences. | **View system consumes AI fields** as displayable columns. |

---

## 21. Open Questions

1. **Multi-entity views** — The current design scopes each view to a single entity type. Should there be a "unified activity feed" view type that displays records from multiple entity types in a single chronological stream? Use case: "Everything related to Contact X" showing Conversations, Communications, Projects, and Action Items together. If so, how does the column system handle heterogeneous entity types in the same grid?

2. **Formula field computation** — Where are formula fields computed? Client-side (fast but limited to loaded records) or server-side (complete but adds query complexity)? Server-side is more correct but makes formulas dependent on the query engine's expression capabilities.

3. **View templates** — Should the system provide pre-built view templates (e.g., "Sales Pipeline", "Stale Relationship Monitor", "Weekly Activity Report") that users can adopt and customize? Could accelerate onboarding.
...
6. **Column formulas vs. field formulas** — Should the view system support view-level computed columns (like spreadsheet formulas that exist only in the view) in addition to entity-level formula fields (which exist on the object and are available across all views)? View-level formulas are more flexible but less reusable.

7. **Saved filter presets** — Should filter configurations be saveable independently of views, so that the same filter preset can be applied to multiple views? Or is duplicating the view sufficient?

8. **Collaborative real-time editing** — If two users are viewing the same shared view and one edits a record inline, should the other see the change in real-time (like Google Sheets)? Or is refresh-on-demand sufficient?
...
10. **Custom entity scope implications** — With per-user custom entities, if User A shares a view of their custom "Jobs" entity with User B, does User B need to have the "Jobs" entity type in their schema to see the view? Or does the shared view carry the schema definition with it?

11. **Relation traversal depth** — Phase 1 limits to one-hop traversal. What's the target maximum depth? Two hops (A → B → C) covers most use cases. Three or more adds significant query complexity. Should there be a hard limit?
...
| Term | Definition |
|---|---|
| **View** | A named, saved configuration defining how to display records of a specific entity type. Includes view type, columns, filters, sorts, and groups. |
