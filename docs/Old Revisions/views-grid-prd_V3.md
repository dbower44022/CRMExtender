...
## CRMExtender â€” Polymorphic View & Grid Subsystem
...
The Views & Grid System is the primary data interaction layer of CRMExtender. It provides users with a polymorphic, entity-agnostic framework for displaying, filtering, sorting, grouping, and editing any entity type in the system â€” whether system-defined (Contacts, Conversations, Communications, Projects, Topics) or user-defined custom objects (Properties, Vehicles, Service Agreements, or anything else a user creates).

The system draws inspiration from tools like ClickUp and Attio, where the same underlying dataset can be rendered through multiple view types â€” a tabular list/grid, a calendar, or a timeline â€” each offering a different lens into the same data. Users create as many views as they need, each with its own configuration of visible columns, filters, sort orders, and groupings.

**Core principles:**

- **Entity-agnostic** â€” The view system does not know or care what entity type it is rendering. It reads from a universal field registry provided by the object model (defined in the Custom Objects PRD). A view of Contacts works identically to a view of a user-defined "Properties" object â€” same column system, same filters, same interactions.
- **Data Source separation** â€” The "what data" layer (Data Source) is separated from the "how to render" layer (View). A Data Source is a reusable, named query â€” built via visual query builder or raw SQL â€” that defines the columns, joins, and base filters. Multiple views can share one Data Source, each rendering the same dataset as a List, Board, Calendar, or Timeline with its own configuration.
- **Cross-entity queries** â€” Data Sources can JOIN across entity types, producing denormalized result sets that combine data from Conversations, Contacts, Companies, Projects, and any custom entities. Views render these cross-entity results seamlessly, with inline editing tracing back to the source entity.
- **Polymorphic columns** â€” Columns adapt based on the entity type being displayed. Each entity type has its own set of fields (system-defined core fields + user-added custom fields), and the column system renders the appropriate editor and display format for each field type.
- **Multiple view types, one dataset** â€” A single entity collection can be viewed as a List/Grid, Board/Kanban, Calendar, or Timeline. Switching view types preserves filters and sort orders where applicable. Each view type is a rendering strategy, not a different data model.
- **User-owned configuration** â€” Every aspect of a view (columns, filters, sorts, groups, column widths, column order) is user-configurable and persisted. Views are personal by default and optionally shareable.
- **Relation-aware** â€” Views can display fields from related entities, not just the primary entity. A Conversations view can show the related Contact's company name, a Project view can show the count of open Conversations â€” traversing the object graph to surface contextual information.
- **Performance-first for real-world scale** â€” Designed for datasets in the low thousands of records, with server-side pagination, virtual scrolling, and query optimization ensuring responsive interaction at scale.

**Relationship to other PRDs:** This PRD defines how data is displayed and interacted with. It depends on the Custom Objects PRD (which defines what entities and fields exist) and is consumed by the Communication & Conversation Intelligence PRD (which defines the system entities that are the primary data sources). The alert system defined in the Conversations PRD is built on top of views â€” an alert is a view with a trigger attached.
...
- **Salesforce** â€” Powerful but complex. List views exist but custom objects require significant configuration. No calendar or timeline views without third-party apps. Views are org-wide, not personal-first.
- **HubSpot** â€” Clean UI but rigid. Custom objects are limited. No relation-traversal columns. Views are basic filtered lists without grouping or aggregation.
- **Attio** â€” Closest to the vision. Flexible object model with rich views. But Attio's view types are limited, and relation columns are somewhat constrained. Strong inspiration, but CRMExtender extends the model.
- **ClickUp** â€” Excellent multi-view paradigm (List, Board, Calendar, Timeline, Gantt). However, ClickUp is task-management-first, not CRM-first. Its data model doesn't support the relationship intelligence and communication hierarchy that CRMExtender requires.
...
1. **Entity-agnostic rendering** â€” Any entity type (system or custom) can be displayed in any supported view type using the same underlying column, filter, sort, and group infrastructure.
2. **Rich column system** â€” Users control which columns are visible, their order, width, and pinning. Columns adapt display and editing behavior based on field type. Relation-traversal columns surface cross-entity context.
3. **Powerful filtering** â€” Users build compound filters with AND/OR logic, using operators appropriate to each field type. Filters can reference fields on related entities.
4. **Flexible sorting & grouping** â€” Multi-level sorting, group-by with collapsible sections, and summary aggregation rows for grouped data.
5. **Multiple view types** â€” The same data renders as a List/Grid (tabular), Calendar (date-plotted), or Timeline (date-range bars). Each view type has type-specific configuration while sharing the common filter/sort infrastructure.
6. **Inline editing** â€” Users edit field values directly in the grid without opening a detail view, for field types that support inline editing.
7. **Persistent, shareable views** â€” Views are saved with all configuration. Personal views are private by default. Shared views are available to team members (with data permissions enforced).
8. **Alert foundation** â€” Views serve as the query definition layer for the alert system. Any view can be promoted to an alert with a trigger, frequency, and delivery method.
...
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Alex â€” Sales Rep**                     | Track active deals, see stale conversations, prep for calls with full context. Uses 2 email accounts, SMS, 30 active deal conversations. | "Show me all Conversations where AI status is Open, grouped by Project, sorted by last activity. Include Contact Company as a column."                                                            |
| **Maria â€” Consultant**                   | Monitor client relationships, surface action items, manage 50+ client relationships across channels.                                     | "Calendar view of all Communications this week. Timeline view of all Projects with start/end dates. List view of Contacts with no activity in 30+ days."                                          |
| **Jordan â€” Team Lead**                   | Oversee team's client conversations, spot escalation needs, weekly reporting. Manages shared inbox.                                      | "Shared view: all Conversations assigned to my team, grouped by owner, with AI Status and Action Item Count columns. Alert me daily on anything Stale + Open."                                    |
| **Sam â€” Gutter Cleaning Business Owner** | Track jobs across multiple cities, manage customer relationships, schedule follow-ups. Custom objects for service areas and jobs.        | "Board view of my custom 'Jobs' object grouped by job status (Scheduled â†’ In Progress â†’ Complete â†’ Invoiced). Calendar view of upcoming jobs. List view filtered by city and service area." |
...
- **US-V14:** As a user, I want to filter by fields on related entities (e.g., "Contact â†’ Company = Acme Corp" on a Conversations view).
...
- **US-V36:** As a user, I want shared views to respect data permissions â€” sharing the view definition doesn't grant access to records the viewer wouldn't otherwise see.
...
Entity Types (from Object Model)
  â”œâ”€â”€ Contacts (con_)     â”€â”€ Field Registry: [Name, Email, Phone, Company, ...]
  â”œâ”€â”€ Conversations (cvr_) â”€â”€ Field Registry: [Subject, AI Status, Last Activity, ...]
  â”œâ”€â”€ Companies (cmp_)     â”€â”€ Field Registry: [Name, Industry, Revenue, ...]
  â”œâ”€â”€ Projects (prj_)      â”€â”€ Field Registry: [Name, Status, Owner, ...]
  â””â”€â”€ Custom: Jobs (job_)  â”€â”€ Field Registry: [Address, Service Type, Price, ...]

Data Sources (reusable query definitions)
  â”œâ”€â”€ Data Source: "Deal Pipeline" (dts_9a2b...)
  â”‚     â”œâ”€â”€ Query: Conversations JOIN Contacts JOIN Companies
  â”‚     â”œâ”€â”€ Columns: [Subject, AI Status, Contact Name, Company, Industry]
  â”‚     â”œâ”€â”€ Default Filter: AI Status â‰  Closed
  â”‚     â”œâ”€â”€ Default Sort: Last Activity DESC
  â”‚     â””â”€â”€ Preview: [Conversation (primary), Contact, Company]
  â”‚
  â”œâ”€â”€ Data Source: "All Contacts" (system-generated)
  â”‚     â””â”€â”€ Query: SELECT * FROM Contacts
  â”‚
  â””â”€â”€ Data Source: "Contact Activity Report" (dts_55fg...)
        â”œâ”€â”€ Query (raw SQL): Contacts LEFT JOIN Conversations (aggregated)
        â”œâ”€â”€ Columns: [Name, Company, Conversation Count, Last Activity]
        â””â”€â”€ Preview: [Contact (primary), Company]

Views (rendering configurations, each referencing a Data Source)
  â”œâ”€â”€ View: "Pipeline Board" (Board) â†’ references "Deal Pipeline" data source
  â”‚     â”œâ”€â”€ Board grouping: AI Status
  â”‚     â”œâ”€â”€ Card fields: [Subject, Contact Name, Company]
  â”‚     â””â”€â”€ Additional filters: Owner = Me
  â”‚
  â”œâ”€â”€ View: "Pipeline List" (List/Grid) â†’ references "Deal Pipeline" data source
  â”‚     â”œâ”€â”€ Visible columns: [Subject, AI Status, Contact Name, Company, Last Activity]
  â”‚     â”œâ”€â”€ Sort override: Company ASC
  â”‚     â””â”€â”€ Group by: AI Status
  â”‚
  â”œâ”€â”€ View: "Expected Closes" (Calendar) â†’ references "Deal Pipeline" data source
  â”‚     â”œâ”€â”€ Date field: Expected Close Date
  â”‚     â””â”€â”€ Card fields: [Subject, Contact Name]
  â”‚
  â””â”€â”€ View: "Contact Report" (List/Grid) â†’ references "Contact Activity Report" data source
        â”œâ”€â”€ Visible columns: [Name, Company, Conversation Count, Last Activity]
        â””â”€â”€ Sort: Conversation Count DESC
```
...
| **Data Source**        | A reusable, named query definition â€” built via visual query builder or raw SQL â€” that produces a structured result set. Defines the entities involved (FROM + JOINs), available columns, default filters, default sort, and previewable entities. Multiple views can share one data source. |
...
| **Relation Traversal** | The ability of a column to display a field from a related entity by following a relation field. Example: a Conversation has a relation field "Primary Contact" pointing to a Contact entity; a column can traverse this relation to display "Primary Contact â†’ Company Name".                 |
...
â”‚  (visible columns, additional filters, sorts, groups, view type)  â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                       â”‚ references
                       â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚                         DATA SOURCE                               â”‚
â”‚  Reusable query definition (visual builder or raw SQL):           â”‚
â”‚  - Defines entity types involved (FROM + JOINs)                   â”‚
â”‚  - Defines available columns (result set schema)                  â”‚
â”‚  - Defines default filters and sort order                         â”‚
â”‚  - Declares previewable entities (auto-detected + optional        â”‚
â”‚    manual override for priority and exclusions)                    â”‚
â”‚  - Multiple views can share one data source                       â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                       â”‚ executes via
                       â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚                         QUERY ENGINE                              â”‚
â”‚  Translates data source + view overrides into executable queries: â”‚
â”‚  - Reads field registry from Object Model                         â”‚
â”‚  - Resolves relation traversals to JOINs                          â”‚
â”‚  - Merges data source defaults with view-level overrides          â”‚
â”‚  - Handles pagination (cursor-based)                              â”‚
â”‚  - Enforces row-level security and tenant isolation               â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                       â”‚
                       â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
...
â”‚  â”‚ List/Grid â”‚  â”‚  Board   â”‚  â”‚ Calendar â”‚  â”‚ Timeline â”‚         â”‚
â”‚  â”‚ Renderer  â”‚  â”‚ Renderer â”‚  â”‚ Renderer â”‚  â”‚ Renderer â”‚         â”‚
â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜         â”‚
â”‚  Each renderer reads the same query results and renders           â”‚
â”‚  according to its view-type-specific rules.                       â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```
...
| **Required entity fields** | None â€” any entity type can be displayed in a grid                 |
...
| **Sorting**                | N/A â€” position is determined by date field value                                                                      |
...
- **Date field mapping** â€” User selects which date field determines calendar position. If the entity has multiple date fields, the user chooses one (e.g., "Due Date" vs. "Created Date").
- **Duration mapping** (optional) â€” If the entity has a start date AND end date (or a duration field), records can span multiple days on the calendar.
- **Card display fields** â€” User selects which fields appear on the calendar card (limited to 1 title field + up to 3 additional fields for space reasons).
- **Color-coding** â€” User selects an optional select/status field to color-code calendar cards.

**When to use:** Date-driven entities where temporal position matters â€” Communications (by timestamp), Tasks/Action Items (by due date), custom objects with date fields (Jobs by scheduled date, Events by event date).
...
- **Start date field mapping** â€” Required. Determines the left edge of the bar.
- **End date field mapping** â€” Optional. Determines the right edge. If not set, records render as point markers (milestones).
- **Sidebar fields** â€” User selects which fields appear in the left sidebar alongside each bar.
- **Color-coding** â€” Same as Calendar: optional select/status field for bar coloring.
- **Swimlane grouping** â€” User selects a group-by field; each unique value gets its own horizontal lane.

**When to use:** Projects (start â†’ end), Conversations (first activity â†’ last activity), custom objects with lifecycle dates, any entity where temporal extent and overlap matter.
...
- **Grouping field** â€” The user selects any single-select field on the entity type. Each option value in the select field becomes a Kanban column. The column order matches the option order defined on the field.
- **Card title field** â€” Which field displays as the card's primary text (defaults to name/subject).
- **Card detail fields** â€” Up to 4 additional fields shown on the card beneath the title.
- **Color-coding** â€” Optional: a second select/status field to color-code card borders or backgrounds. If not set, cards inherit the color of the Kanban column they're in (from the select field's option colors).
- **Swimlane field** (optional) â€” A second group-by field that creates horizontal rows across all columns. Example: group Kanban columns by Deal Stage, swimlanes by Owner â€” creating a matrix of Owner Ã— Stage.
- **Card size** â€” Compact (title only), Standard (title + 2 fields), or Expanded (title + 4 fields).

**When to use:** Status-driven workflows where progression through stages is the primary interaction â€” deal pipelines, project phases, task workflows, approval processes. Any entity with a single-select field representing a lifecycle or status.
...
Example: On a Conversations view, a "Contact â†’ Company" column follows the `primary_contact` relation field to the Contact entity, then displays that Contact's `company` field.

Traversal depth is limited to **one hop** in Phase 1 (primary entity â†’ related entity). Multi-hop traversal (primary â†’ related â†’ related-of-related) is deferred to Phase 2.
...
- **Record count:** "Open Conversation Count" on a Contacts view â€” counts the number of Conversations with `ai_status = open` related to this Contact.
- **Formula field:** "Days Since Last Activity" â€” calculates the difference between today and the `last_activity` timestamp.
- **Rollup:** "Total Deal Value" on a Project view â€” sums the `value` field across all related Deal records.
...
2. **Custom entities** default to: Name/Title field (first text field), Status field (if exists), Created Date, Last Modified Date â€” plus the first 2-3 custom fields in the order they were defined.
...
| ------------------ | ----------------------------------------------------------------------------------- | ------------------------------------------ |
| **Field mapping**  | Which field (or relationâ†’field) this column displays                              | Set at column creation                     |
...
| **Sort**          | Click column header to toggle sort (none â†’ ASC â†’ DESC â†’ none). Shift+click to add as secondary sort.                  |
...
The view system renders columns, filters, and editors based on field types. Each field type defines its display renderer, edit widget, and applicable filter operators. The field types themselves are defined by the Custom Objects PRD â€” this section specifies how the view system handles each type.
...
| **Text (multi-line)**  | First line + "..." indicator                                 | Expandable textarea (opens in popup)  | Yes (alphabetical)                      | No â€” too many unique values                                     |
...
| **Formula**            | Computed value (renders based on output type)                | Read-only â€” not editable            | Yes (based on output type)              | Yes (based on output type)                                        |
| **Rollup**             | Aggregated value from related records                        | Read-only â€” not editable            | Yes (numeric)                           | Yes                                                               |
...
| `is_within_last N weeks`  | Today minus NÃ—7 days through today     |
| `is_within_last N months` | Today minus N months through today      |
| `is_within_next N days`   | Today through today plus N days         |
| `is_within_next N weeks`  | Today through today plus NÃ—7 days      |
| `is_within_next N months` | Today through today plus N months       |

Relative dates are critical for saved views and alerts â€” a "Stale Conversations" view filtered by "Last Activity is_within_last 14 days = false" stays current without manual adjustment.
...
Relation traversal allows a view to display fields that don't belong to the primary entity type but to a related entity. This is one of the most powerful features of the view system â€” it surfaces cross-entity context without requiring the user to navigate between screens.

### 10.2 How It Works

```
Primary Entity: Conversation
  â””â”€â”€ Field: primary_contact (Relation â†’ Contact)

Related Entity: Contact
  â”œâ”€â”€ Field: company (Text)
  â”œâ”€â”€ Field: email (Email)
  â””â”€â”€ Field: last_activity (Datetime)

Lookup Column: "Primary Contact â†’ Company"
  Step 1: Read the Conversation's primary_contact relation field â†’ get Contact ID
  Step 2: Read the referenced Contact's company field â†’ get "Acme Corp"
...
| Rule               | Detail                                                                                                                                                                                                                             |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Depth limit**    | One hop in Phase 1 (A â†’ B). Multi-hop (A â†’ B â†’ C) deferred to Phase 2.                                                                                                                                                       |
...
| **Null handling**  | If the relation field is empty (no related entity), the lookup column displays "â€”" (em-dash).                                                                                                                                    |
...
```
Filter Root (AND)
â”œâ”€â”€ Condition: Status = "Active"
â”œâ”€â”€ Condition: Last Activity > "2026-01-01"
â””â”€â”€ Group (OR)
      â”œâ”€â”€ Condition: Priority = "High"
      â””â”€â”€ Condition: Assignee = Me
```
...
| ------------ | ------------------------------------------------------------ | ---------------------------------------- |
| **Field**    | The field to filter on (direct or relation traversal)        | `ai_status`, `primary_contactâ†’company` |
...
Level 0: Root group (AND/OR)
  Level 1: Nested group
    Level 2: Nested group
      Level 3: Deepest allowed nesting â€” conditions only, no further groups
```

### 11.3 Quick Filters

For common, frequently used filter patterns, the view provides quick filter controls above the grid â€” single-click shortcuts that add pre-configured filter conditions:
...
Quick filters are additive â€” they add to existing filter conditions (AND logic). They can be toggled on/off without opening the full filter builder.
...
```
Filter: Primary Contact â†’ Company  equals  "Acme Corp"
```
...
- **Click column header:** Toggle between ASC â†’ DESC â†’ Remove sort.
...
```
â–¼ Project: Acme Acquisition (3 conversations)
...
â–¼ Project: Office Relocation (2 conversations)
    Conversation: Lease Negotiation  Open     Feb 15
    Conversation: Moving Logistics   Closed   Feb 8

â–¶ Project: (Unassigned) (5 conversations)  [collapsed]
```
...
| **Range**     | Date fields                        | "Feb 8 â€“ Feb 15" |
...
Bulk actions display a floating action bar at the bottom of the screen when records are selected: "15 records selected â€” [Edit] [Assign] [Delete] [Export]".
...
â”‚  Feb 9  â”‚  Feb 10 â”‚  Feb 11 â”‚  Feb 12 â”‚  Feb 13 â”‚  Feb 14 â”‚  Feb 15 â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚         â”‚ â”Œâ”€â”€â”€â”€â”€â” â”‚         â”‚ â”Œâ”€â”€â”€â”€â”€â” â”‚ â”Œâ”€â”€â”€â”€â”€â” â”‚         â”‚         â”‚
â”‚         â”‚ â”‚ Bob â”‚ â”‚         â”‚ â”‚Due  â”‚ â”‚ â”‚Call â”‚ â”‚         â”‚         â”‚
â”‚         â”‚ â”‚mtg  â”‚ â”‚         â”‚ â”‚Dili â”‚ â”‚ â”‚w/   â”‚ â”‚         â”‚         â”‚
â”‚         â”‚ â”‚ ðŸŸ¢  â”‚ â”‚         â”‚ â”‚ ðŸŸ¡  â”‚ â”‚ â”‚Sara â”‚ â”‚         â”‚         â”‚
â”‚         â”‚ â””â”€â”€â”€â”€â”€â”˜ â”‚         â”‚ â””â”€â”€â”€â”€â”€â”˜ â”‚ â”‚ ðŸ”´  â”‚ â”‚         â”‚         â”‚
...
â”‚  Prospect   â”‚ Negotiation â”‚  Proposal   â”‚   Closed    â”‚
â”‚  (4 cards)  â”‚  (2 cards)  â”‚  (3 cards)  â”‚  (6 cards)  â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”‚ â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”‚ â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”‚ â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”‚
â”‚ â”‚ Acme    â”‚ â”‚ â”‚ Beta Co â”‚ â”‚ â”‚ Delta   â”‚ â”‚ â”‚ Epsilon â”‚ â”‚
â”‚ â”‚ $50K    â”‚ â”‚ â”‚ $120K   â”‚ â”‚ â”‚ $80K    â”‚ â”‚ â”‚ $200K   â”‚ â”‚
â”‚ â”‚ Alex    â”‚ â”‚ â”‚ Maria   â”‚ â”‚ â”‚ Alex    â”‚ â”‚ â”‚ Jordan  â”‚ â”‚
â”‚ â”‚ ðŸŸ¢ Hot  â”‚ â”‚ â”‚ ðŸŸ¡ Warm â”‚ â”‚ â”‚ ðŸŸ¢ Hot  â”‚ â”‚ â”‚ ðŸŸ¢ Won  â”‚ â”‚
â”‚ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜ â”‚ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜ â”‚ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜ â”‚ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜ â”‚
â”‚ â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”‚ â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”‚ â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”‚ â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”‚
â”‚ â”‚ Gamma   â”‚ â”‚ â”‚ Zeta    â”‚ â”‚ â”‚ Theta   â”‚ â”‚ â”‚ Iota    â”‚ â”‚
â”‚ â”‚ $30K    â”‚ â”‚ â”‚ $75K    â”‚ â”‚ â”‚ $45K    â”‚ â”‚ â”‚ $150K   â”‚ â”‚
â”‚ â”‚ Maria   â”‚ â”‚ â”‚ Alex    â”‚ â”‚ â”‚ Jordan  â”‚ â”‚ â”‚ Alex    â”‚ â”‚
â”‚ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜ â”‚ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜ â”‚ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜ â”‚ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜ â”‚
...
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Sum: $145K  â”‚ Sum: $195K  â”‚ Sum: $185K  â”‚ Sum: $890K  â”‚
...
When a swimlane field is configured, the board is divided into horizontal rows â€” one per unique value in the swimlane field. Each swimlane contains its own set of columns, creating a matrix:

```
                 Prospect    Negotiation    Closed
               â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
  Owner: Alex  â”‚ [Acme]    â”‚ [Zeta]     â”‚ [Iota]   â”‚
               â”‚ [Gamma]   â”‚            â”‚          â”‚
               â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
  Owner: Maria â”‚ [Beta]    â”‚            â”‚ [Kappa]  â”‚
               â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
  Owner: Jordanâ”‚           â”‚ [Delta]    â”‚ [Epsilon]â”‚
...
If a select field option has no matching records (after filters are applied), the column still renders â€” but as an empty column with a "+" button to create a new record in that state. This ensures the full workflow pipeline is always visible, even when some stages are empty.

### 15.8 Unset / Null Handling

Records where the grouping field has no value (null/empty) are collected in a special "(No Status)" column, displayed as the first or last column (user-configurable). This column behaves identically to named columns â€” cards can be dragged out of it to assign a status, or into it to clear the status.
...
```
                     Feb 2026
Sidebar          â”‚ W1  â”‚ W2  â”‚ W3  â”‚ W4  â”‚ Mar â”‚
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¤
Acme Acquisition â”‚ â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘ â”‚     â”‚
                 â”‚     â”‚     â”‚     â”‚     â”‚     â”‚
Office Relo.     â”‚     â”‚ â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ”‚
                 â”‚     â”‚     â”‚     â”‚     â”‚     â”‚
Q1 Hiring        â”‚â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ”‚     â”‚     â”‚     â”‚
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¤
```
...
When a swimlane field is configured, the timeline is divided into horizontal lanes â€” one per unique value in the group-by field. This is the Timeline equivalent of grouping in the List view.

```
                         Feb 2026
                    â”‚ W1  â”‚ W2  â”‚ W3  â”‚ W4  â”‚

â–¼ Owner: Alex      â”‚     â”‚     â”‚     â”‚     â”‚
  Acme Acquisition  â”‚ â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘ â”‚
  Q1 Hiring         â”‚â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ”‚     â”‚     â”‚

â–¼ Owner: Maria     â”‚     â”‚     â”‚     â”‚     â”‚
  Office Relo.      â”‚     â”‚ â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ”‚
```
...
Conversations
  ...
  â”‚ [My Open Convos â–¼] [All Convos] [Stale VIPs] [Team Review*] â”‚ + New View
...
View: "Stale VIP Conversations"
  ...
  Filters: Contactâ†’Tag contains "VIP" AND Last Activity is_within_last 14 days = false
  Sort: Last Activity ASC
  â”‚
  â””â”€â”€ Alert (optional attachment)
        â”œâ”€â”€ Trigger: "New records match" (when a Conversation newly matches the filter)
        â”œâ”€â”€ Frequency: Daily
        â”œâ”€â”€ Aggregation: Batched (digest)
        â””â”€â”€ Delivery: Email
```
...
| Parameter        | Value                                                                                                      |
| ---------------- | ---------------------------------------------------------------------------------------------------------- |
| **Page size**    | 50 records (default) â€” configurable: 25, 50, 100                                                         |
...
- Row click to open detail page (single entity type â†’ auto-primary preview)
...
- Visual builder â†’ SQL "eject" workflow
...
1. **Multi-entity views vs. cross-entity data sources** â€” With Data Sources now supporting JOINs across entity types, the original "multi-entity view" question is partially addressed. A data source joining Conversations + Contacts + Companies produces a cross-entity result set. However, this is still a flattened/denormalized view. Should there also be a "unified activity feed" view type that displays heterogeneous entity types (a Conversation row, then a Communication row, then a Project row) in a single chronological stream? This would require a fundamentally different rendering model where each row can have a different column schema.

2. **Formula field computation** â€” Where are formula fields computed? Client-side (fast but limited to loaded records) or server-side (complete but adds query complexity)? Server-side is more correct but makes formulas dependent on the query engine's expression capabilities. With raw SQL data sources, users can compute formulas in the query itself (CASE expressions, window functions) â€” does this reduce the need for a separate formula field type?

3. **View templates** â€” Should the system provide pre-built view templates that include both a data source and a view configuration (e.g., a "Sales Pipeline" template that creates a cross-entity data source + Board view)? Could accelerate onboarding.

4. **Export formats** â€” Beyond CSV, should views support export to Excel (.xlsx), PDF, or direct integration with reporting tools? What about scheduled exports (e.g., "email me this view as a CSV every Monday")?

5. **Conditional formatting** â€” Should cells/rows support conditional formatting rules (e.g., "if AI Status = Open, highlight row in yellow", "if Days Since Last Activity > 30, make cell red")? This is powerful but adds significant UI complexity.

6. **Column formulas vs. field formulas** â€” Should the view system support view-level computed columns (like spreadsheet formulas that exist only in the view) in addition to entity-level formula fields (which exist on the object and are available across all views)? With raw SQL data sources, users can already create computed columns via SQL expressions â€” is a UI-based formula builder additionally needed?

7. **Saved filter presets** â€” Should filter configurations be saveable independently of views, so that the same filter preset can be applied to multiple views? Or is the Data Source's default filter + view-level filter composition sufficient?

8. **Collaborative real-time editing** â€” If two users are viewing the same shared view and one edits a record inline, should the other see the change in real-time (like Google Sheets)? Or is refresh-on-demand sufficient? This is particularly complex with cross-entity data sources where an edit to a Company record should propagate to all rows referencing that Company.

9. **Print / presentation mode** â€” Should views have a "print-friendly" rendering that removes interactive controls and optimizes for paper or screen sharing? Relevant for managers presenting data in meetings.

10. **Custom entity scope implications** â€” With per-user custom entities, if User A shares a data source that queries their custom "Jobs" entity, can User B (who doesn't have a "Jobs" entity) use a view on that data source? The data source defines the schema â€” does the viewer need the entity type in their own schema, or does the data source's virtual schema suffice?

11. **Relation traversal vs. data source JOINs** â€” The Relation Traversal system (Section 10) and Data Source JOINs both achieve cross-entity column display. Should relation traversal remain as a convenience feature within single-entity data sources (auto-generating the JOIN under the hood), or should it be deprecated in favor of explicit data source JOINs? Recommendation: keep both â€” relation traversal is simpler for common cases; data source JOINs are more powerful for complex cases.

12. **Mobile rendering** â€” How do List/Grid, Board, Calendar, and Timeline views render on mobile devices? The List view likely needs a card-based responsive rendering. Board may need a single-column swipeable layout. Calendar and Timeline may need simplified mobile-specific layouts.

13. **Data source marketplace** â€” Should there be a community or organization-level library of shared data source templates? Power users create useful queries; other users browse and adopt them for their own views.

14. **SQL guardrails** â€” What limits should be placed on raw SQL complexity? Maximum number of JOINs? Maximum query cost estimate? Should the system prevent queries that would scan entire large tables without index-aligned filters?

15. **Data source lineage & impact analysis** â€” When a data source is about to be edited, should the system show which views depend on it and what the impact of the change will be? A visual dependency graph could help authors understand the blast radius of their changes.
...
| **Virtual Schema**                            | The logical representation of entity types and fields that raw SQL queries execute against. Users never see the physical database schema â€” only entity types as tables and fields as columns.                                                    |
...
| **Swimlane**                                  | A horizontal lane in a Timeline or Board view, created by grouping records by a field value. In Board views, swimlanes create a matrix of group-by Ã— status.                                                                                      |
