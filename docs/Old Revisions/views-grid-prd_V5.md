...
When more than one row is selected, the grid and Content Tool Bar switch into Bulk Action Mode.  This mode allows users to perform only the appropriate actions for Bulk operations.  The available bulk operations are unique to each Entity Type and are defined below.
