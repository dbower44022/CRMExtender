...
> **V1.0 (2026-02-17):**
> Initial specification. Defined unified object model, field registry, field type system, field groups, relation types, event sourcing, DDL management, and schema-per-tenant architecture.
>
> **V1.1 (2026-02-21):**
> Terminology alignment with Master Glossary V3 and GUI Functional Requirements PRD V2. Updated Section 7.2 (Display Name Field) usage list to reference Card-Based Architecture terms (Identity Card, Activity Card). Updated Section 11 (Field Groups) to note that each Field Group renders as an Attribute Card in the Detail Panel's Card Layout Area. Updated Section 19.5 to reference Detail Panel. Updated PRD links to current versions. Added cross-reference to Master Glossary V3 in glossary. No behavioral changes.
...
- Identity Card header (Detail Panel, GUI PRD Section 15)
- Board view card titles
- Calendar view event labels
- Search result labels
- Activity Card references (GUI PRD Section 15.6)
...
- Detail Panel rendering (Attribute Card layout)
...
| `name` | TEXT, NOT NULL | Display name (e.g., "Basic Info", "Pricing", "Scheduling") — becomes the Attribute Card header |
| `description` | TEXT | Optional description |
| `display_order` | INTEGER | Position among Attribute Cards in the Card Layout Area |
| `is_collapsed_default` | BOOLEAN | Whether the Attribute Card starts collapsed in the Detail Panel |

### 11.2 Behavior

- Fields not assigned to any group appear in a default "General" Attribute Card at the top of the Card Layout Area
- Each Field Group renders as a collapsible Attribute Card in the Detail Panel (GUI PRD Section 15.7)
- Each Attribute Card displays its member fields in their `display_order` sequence
- Attribute Cards can be reordered via drag-and-drop in entity type settings
- Empty groups (no fields assigned) are hidden from the Detail Panel
- Universal fields (id, created_at, etc.) appear in a system-rendered "Record Info" section, not in user-defined Attribute Cards
...
- Field Groups rendered as Attribute Cards in the Detail Panel
...
| **Field Group** | A named grouping of fields for organizing the Detail Panel layout. Each Field Group renders as an Attribute Card (GUI PRD Section 15.7). Purely presentational. |
