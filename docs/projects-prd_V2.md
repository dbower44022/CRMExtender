...
## CRMExtender â€” Central Organizational Hub

**Version:** 2.0
**Date:** 2026-02-19
**Status:** Draft â€” Reconciled with Custom Objects PRD
...
> - **Projects are the central organizational hub** â€” not just a conversation container. Events, Notes, Contacts, Companies, and Custom Objects all link directly to Projects.
> - **Flexible hierarchy** â€” Conversations can belong to a Project directly OR via a Topic. Topics can exist independently (no longer required to belong to a Project). All levels are optional.
> - **User-defined status workflow** â€” No system-imposed status states. Users define their own status options and transitions. The only system-managed state is `archived_at` (the universal field).
> - **Explicit Contact and Company associations** â€” Projectâ†”Contact and Projectâ†”Company are first-class relations, not derived from communication participants.
...
The Project subsystem is the **central organizational hub** of CRMExtender. A Project represents a business initiative, engagement, deal, campaign, or any purposeful body of work. While other entities capture specific types of data â€” Communications capture messages, Conversations group threads, Events track meetings, Contacts represent people â€” the Project ties them all together into a coherent picture of "what is this work, who is involved, and what has happened."
...
- **Central hub, not conversation container** â€” Projects organize all work, not just communications. A project links to contacts who may never send a message, events that have no associated emails, and notes capturing strategic context that exists outside any conversation.
- **User-defined workflow** â€” The platform imposes no status states or transitions. Users define their own project lifecycle that matches their business reality. A gutter cleaning company and a venture capital fund have fundamentally different project workflows â€” the platform accommodates both without opinion.
- **Flexible hierarchy** â€” Conversations link to Projects via the Conversation↔Project system Relation Type (many-to-many). Aggregate Conversations provide organizational grouping within or outside of Projects. Sub-projects can nest to any depth. Every connection is optional â€” the hierarchy helps users organize, it never forces structure.
- **Explicit relationships** â€” Contacts and Companies are intentionally associated with Projects, not derived from communication patterns. A project's stakeholder list represents who matters, not just who has emailed.
- **System Relation Types ship out of the box** â€” The most common entity connections are pre-defined, eliminating setup friction while preserving user extensibility through Custom Objects.
- **Event-sourced history** â€” All mutations to Projects are stored as immutable events, enabling full audit trails and point-in-time reconstruction.

**Relationship to other PRDs:**

- **[Custom Objects PRD](custom-objects-prd.md)** â€” Project is a system object type. Table structure, field registry, event sourcing, and relation model are governed by the Custom Objects framework. This PRD defines the entity-specific behaviors and relations.
- **[Conversations PRD](conversations-prd_V3.md)** â€” Conversations link to Projects via the Conversation↔Project system Relation Type (many-to-many). The Conversations PRD defines the Conversation entity (standard and aggregate); this PRD defines the Project side of those relationships. Aggregate Conversations replace the former Topic entity.
- **[Communications PRD](communications-prd_V2.md)** â€” Communications belong to Conversations, which may belong to Projects. There is no direct Projectâ†’Communication relation.
- **[Contact Management PRD](contact-management-prd_V4.md)** â€” Contacts link to Projects through an explicit system Relation Type. Project association is independent of communication participation.
- **[Company Management PRD](company-management-prd.md)** â€” Companies link to Projects through an explicit system Relation Type.
- **[Event Management PRD](events-prd_V2.md)** â€” Events link to Projects through a system Relation Type.
- **[Notes PRD](notes-prd_V2.md)** â€” Notes attach to Projects through the Universal Attachment Relation pattern.
- **[Data Sources PRD](data-sources-prd.md)** â€” Virtual schema table for Projects is derived from the Project field registry.
- **[Views & Grid PRD](views-grid-prd_V3.md)** â€” Project views, filters, sorts, dashboards, and inline editing operate on fields from the Project field registry.
- **[Permissions & Sharing PRD](permissions-sharing-prd_V1.md)** â€” Project record access follows the standard role-based access model.
...
| **Contact association limited to communication** | CRMs only know about relationships where communication has occurred. A project stakeholder who hasn't emailed you â€” a zoning attorney, a general contractor, a silent investor â€” is invisible in the project context. |
| **Rigid project models** | Tools that do offer "projects" or "deals" impose fixed status workflows (Lead â†’ Qualified â†’ Proposal â†’ Closed) that don't match every business. A gutter cleaning company's project lifecycle differs fundamentally from a consulting engagement or a real estate transaction. |
| **No hierarchical organization** | Complex initiatives with multiple workstreams â€” an expansion with NYC, London, and Infrastructure sub-projects â€” cannot be modeled as a hierarchy. Users resort to naming conventions or separate tools. |
...
1. **Central organizational hub** â€” Every major entity type can be linked to a Project, providing a single place to see all related work.
2. **User-defined workflow** â€” Users define their own project status states and transitions, matching their actual business processes.
3. **Flexible hierarchy** â€” Sub-projects, Conversation↔Project Relation Type links, and explicit contact/company associations all coexist.
4. **Zero-friction setup** â€” System Relation Types for core entity connections ship out of the box. Users don't need to configure anything to start organizing work around Projects.
5. **Extensible** â€” Users can add custom fields to Projects and create relations to Custom Object Types, making Projects work for any domain.
...
| Entity linking breadth | Average project has links to â‰¥3 entity types | DB query: distinct relation types per project |
...
|---|---|---|
| **Sam â€” Gutter Business Owner** | Multiple expansion projects across cities. Each involves properties, jobs, contractors, permits, and supplier relationships. | Organize work by city expansion. Link contacts (contractors, inspectors) to projects before any communication occurs. Track project status through his custom workflow (scoping â†’ scheduled â†’ in_progress â†’ invoiced â†’ closed). |
| **Alex â€” Sales Rep** | 30 active deals, each involving multiple stakeholders, conversations, meetings, and proposal documents. | Link all conversations for a deal to one project. Associate key decision-makers even if they haven't communicated directly. Track deal stage through a custom sales pipeline. |
| **Maria â€” Consultant** | Long-running client engagements with multiple workstreams. Each workstream has its own conversations, deliverables, and meetings. | Sub-projects for each workstream. Link events (client meetings) directly to the engagement project. Notes capturing strategic context attached to the project. |
...
- **US-11:** As a user, I want a project detail view that shows all linked entities â€” conversations, contacts, events, notes â€” in one place.
...
| `type_prefix` | `prj_` |
| `is_system` | `true` |
| `display_name_field_id` | â†’ `name` field |
...
| User Status | `user_status` | Select | NO | User-defined workflow status. Options and transitions configured by the user (see Section 9). No default options â€” user populates their own. |
| Owner ID | `owner_id` | Relation (â†’ users) | NO | The user who manages the project. |
| Parent Project ID | `parent_project_id` | Relation (â†’ projects) | NO | Self-referential FK for sub-project nesting. NULL for top-level projects. |
...
**Note:** There is no system-managed status field. The `archived_at` universal field (present on all entity types) serves as the sole system lifecycle state â€” a project is either active (`archived_at IS NULL`) or archived (`archived_at IS NOT NULL`). All workflow-level status management is delegated to the user-defined `user_status` select field.
...
Project: 2026 Expansion
  â”œâ”€â”€ Sub-project: NYC Office
  â”‚     â”œâ”€â”€ Aggregate: Real Estate
  â”‚     â”œâ”€â”€ Aggregate: Hiring
  â”‚     â””â”€â”€ Aggregate: Regulatory
  â”œâ”€â”€ Sub-project: London Office
  â”‚     â”œâ”€â”€ Aggregate: Real Estate
  â”‚     â””â”€â”€ Aggregate: Visa Sponsorship
  â””â”€â”€ Sub-project: Infrastructure
        â”œâ”€â”€ Aggregate: Cloud Migration
        â””â”€â”€ Aggregate: Security Audit
```

### 6.3 Depth

Sub-project nesting depth is **unlimited**. The self-referential Relation Type naturally supports any depth. In practice, users rarely go beyond 2â€“3 levels. No artificial depth limit is imposed â€” if a user wants "Expansion â†’ NYC â†’ Legal â†’ Contract Review," that is their organizational choice.

### 6.4 Cascade Behavior

When a project is archived, all child sub-projects are cascaded to archived status. This follows the Relation Type's cascade configuration. Unarchiving a parent does **not** automatically unarchive children â€” the user must explicitly restore each sub-project they want active.
...
### 8.6 Project â†” Project (Sub-Project Hierarchy)
...
The Custom Objects creation UX surfaces relationship definition as part of the setup flow, allowing users to easily establish Project â†” Custom Object relations at creation time.
...
|---|---|
| Gutter cleaning | `scoping` â†’ `scheduled` â†’ `in_progress` â†’ `invoiced` â†’ `closed` |
| Real estate investing | `due_diligence` â†’ `under_contract` â†’ `closing` â†’ `post_closing` |
| Consulting | `proposal` â†’ `active` â†’ `review` â†’ `completed` |
| Sales pipeline | `qualified` â†’ `demo` â†’ `negotiation` â†’ `won` / `lost` |
...
- **Proactive** â€” User creates a project before any communications occur, in anticipation of future work. May create aggregate Conversations, associate contacts and companies. Example: "2026 Cleveland Expansion" project created with stakeholder contacts and sub-projects before any emails are exchanged.
- **Reactive** â€” User creates a project in response to an incoming communication that introduces a new initiative. Existing conversations and contacts are linked retroactively.
...
- `projects_events` â€” All mutations to Project records
...
- `projects` â€” All project fields (universal + core system + user-defined custom fields)
...
Same rationale as the original Conversations PRD: the self-referential Relation Type naturally supports any depth. In practice, users rarely go beyond 2â€“3 levels. Adding an artificial depth limit requires enforcement logic without meaningful benefit.
...
- Project â†” Conversation direct link (new `project_id` on conversations)
- Project â†” Contact explicit association (junction table)
- Project â†” Company explicit association (junction table)
...
- Project â†” Event link (junction table)
- Project â†” Note link (universal attachment pattern)
...
- Custom Object Type â†’ Project relation creation in the object type setup flow
...
| **Communications PRD** | Communications belong to Conversations, which may belong to Projects. No direct Projectâ†’Communication relation. | **Indirect.** Projects access communications through conversations. |
...
1. **Sub-project depth soft warning** â€” Should the UI display a soft warning when sub-project nesting exceeds 3â€“4 levels? No enforcement, just a "this is getting deep" indicator.

2. **Archive cascade to linked entities** â€” When a project is archived, sub-projects are cascaded. Should any other linked entities be affected? Current decision: no â€” conversations, contacts, events, notes persist independently. 

3. **Project templates** â€” Should the system support pre-defined project structures (status states, aggregate Conversations, sub-projects) that users can instantiate? Example: a "New City Expansion" template with pre-configured sub-projects and aggregate Conversations.

4. **Bulk entity linking** â€” Should the API support batch operations for linking multiple conversations, contacts, or events to a project in a single request?

5. **Derived participant display** â€” The project detail view should show both explicitly associated contacts and derived participants from linked conversations. Should derived participants be visually distinguished? Should there be a one-click action to promote a derived participant to an explicit association?

6. **Project merge/split** â€” Can two projects be merged? Can a project be split into sub-projects? What happens to linked entities during these operations?

7. **Cross-tenant project sharing** â€” Future consideration for multi-tenant scenarios: can a project be shared across tenant boundaries?

---

## 18. Future Work

- **Project templates** â€” Pre-defined project structures for common workflows, instantiable with one click.
- **Project analytics** â€” Activity metrics, communication frequency, entity linkage density, timeline visualization.
- **AI project health signals** â€” User-configurable AI analysis of linked conversation sentiment, stale conversations, and unresolved action items at the project level.
- **Project cloning** â€” Duplicate a project's structure (sub-projects, aggregate Conversation structures, status configuration) without data for repeatable workflows.
- **Timeline view** â€” Chronological visualization of all activity across a project's linked entities.
- **Project-level permissions** â€” Fine-grained access control at the project level (who can see which projects and their linked entities).
- **Automation triggers** â€” User-defined automations triggered by project status transitions (e.g., "when status changes to 'invoiced', create a Job record").
...
| **User Status** | A user-defined Select field on the Project entity representing the project's workflow state. Options and transitions are configured by the user â€” the platform imposes no default states. |
