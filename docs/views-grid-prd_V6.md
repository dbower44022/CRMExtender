...
When more than one row is selected, the grid and Content Tool Bar switch into Bulk Action Mode.  This mode allows users to perform only the appropriate actions for Bulk operations.  The available bulk operations are unique to each Entity Type and are defined in each entities PRD.  The general list below should be used if a PRD does not have an Available Actions section.
...
| Add Tag       | Assign a Tag to all Entities selected.    |
| Add Relation  | Select a Relation Type and Target         |
