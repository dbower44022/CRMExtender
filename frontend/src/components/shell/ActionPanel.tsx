...
function SettingsPanel() {
  ...
  const generalTabs = SETTINGS_TABS.filter((t) => !t.adminOnly)
  const adminTabs = SETTINGS_TABS.filter((t) => t.adminOnly)

  return (
    <div className="flex h-full flex-col overflow-hidden border-r border-surface-200 bg-surface-50">
      <div className="flex items-center border-b border-surface-200 px-3 py-2.5">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1">
        <div className="mb-2">
          {generalTabs.map((tab) => {
            const Icon = tab.icon
            const isActive = tab.key === settingsTab
            return (
              <button
                key={tab.key}
        ...
        {isAdmin && (
          <>
            <div className="mx-2 my-2 h-px bg-surface-200" />
